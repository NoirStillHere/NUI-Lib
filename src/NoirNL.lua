...
NoirUI.BuiltInRegular = Font.new('rbxasset://LuaPackages/Packages/_Index/BuilderIcons/BuilderIcons/BuilderIcons.json',Enum.FontWeight.Regular,Enum.FontStyle.Normal);
NoirUI.BuiltInBold = Font.new('rbxasset://LuaPackages/Packages/_Index/BuilderIcons/BuilderIcons/BuilderIcons.json',Enum.FontWeight.Bold,Enum.FontStyle.Normal);
NoirUI.GlobalSignals = {};
NoirUI.UnloadEnabled = false;
...
NoirUI.UserProfile = Players:GetUserThumbnailAsync(LocalPlayer.UserId , Enum.ThumbnailType.HeadShot , Enum.ThumbnailSize.Size150x150)
NoirUI.RandomString = LPH_NO_VIRTUALIZE(function()
...
GlobalWindow.Name = NoirUI.RandomString();
...
NoirUI.IconColor = Color3.fromRGB(255, 255, 255);
NoirUI.ScreenGui = GlobalWindow;
NoirUI.Flags = {};
NoirUI.AccentColor = Color3.fromRGB(78, 127, 252);
NoirUI.MainColor = Color3.fromRGB(8, 8, 13);
NoirUI.RegisiteryColor = {};
NoirUI.NameRegisitry = {};
NoirUI.IsMosueOverOtherFrame = false;
NoirUI.GlobalLogo = "rbxthumb://type=Asset&id=94916110511437&w=420&h=420";
NoirUI.ImageColorMapping = "rbxassetid://4155801252";

if getcustomasset then
	local link = "https://github.com/NoirStillHere/NUI-Lib/blob/main/assets/%s?raw=true";
	local dir = 'NAAssets';
	...
	pcall(function()
		...
		if isfile(dir..'/'..'logo.png') then
			NoirUI.GlobalLogo = getcustomasset(dir..'/'..'logo.png')
		end;
	end);
	...
	pcall(function()
		...
		if isfile(dir..'/'..'saturation_value_gradient.png') then
			NoirUI.ImageColorMapping = getcustomasset(dir..'/'..'saturation_value_gradient.png')
		end;
	end);
end;

function NoirUI:AddSignal(RBXSignal)
	if NoirUI.UnloadEnabled then
		table.insert(NoirUI.GlobalSignals,RBXSignal);
	end;

	return RBXSignal;
end;

function NoirUI:AddQuery(ItemRoot: Frame , Name : string)
	table.insert(NoirUI.NameRegisitry , {
		Root = ItemRoot,
		Idx = Name,
	});
end;
...
do
	...
	NoirUI.Base64Encode = LPH_NO_VIRTUALIZE(function(data)
	...
	NoirUI.Base64Decode = LPH_NO_VIRTUALIZE(function(data)
...
NoirUI.LoadIcon = LPH_NO_VIRTUALIZE(function()
	NoirUI.RobloxIcon = {
...
NoirUI.IsMouseOverFrame = LPH_NO_VIRTUALIZE(function(self , Frame)
	if not Frame then
		return;
	end;

	if NoirUI.Global3DRenderMode then
...
NoirUI.CreateSignal = LPH_NO_VIRTUALIZE(function(self , DefaultValue)
...
NoirUI.SetIconMode = LPH_NO_VIRTUALIZE(function(self , Label: TextLabel , Icon: string)
	local useBold = string.lower(string.sub(Icon , -5)) == '-bold';

	if useBold then
		Label.Text = Icon:sub(1,-6);
		Label.FontFace = NoirUI.BuiltInBold;
	else
		Label.Text = Icon;
		Label.FontFace = NoirUI.BuiltInRegular;
	end;
end);

function NoirUI:GetIconFont(icon: string)
	local useBold = string.lower(string.sub(icon , -5)) == '-bold';

	if useBold then
		return NoirUI.BuiltInBold;
	end;

	return NoirUI.BuiltInRegular;
end;

function NoirUI:MoreThanHalfY(Value: number)
	return (NoirUI.ScreenGui.AbsoluteSize.Y / 2) < Value
end;

NoirUI.IsStudio = RunService:IsStudio();
NoirUI.IsMobile = UserInputService.TouchEnabled;

NoirUI.CreateInput = LPH_NO_VIRTUALIZE(function(self , Frame , Callback)
...
NoirUI.PlayAnimate = LPH_NO_VIRTUALIZE(function(Self , Info , Property)
...
NoirUI.Drag = LPH_NO_VIRTUALIZE(function(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local updateInput = function(input)
		...
		if NoirUI.Global3DRenderMode then
			NoirUI.PlayAnimate(MoveFrame,Tween,{
	...
	NoirUI:AddSignal(InputFrame.InputBegan:Connect(function(input)
	...
	NoirUI:AddSignal(UserInputService.InputChanged:Connect(function(input)
...
NoirUI.Rounding = LPH_NO_VIRTUALIZE(function(num, numDecimalPlaces)
	local mult = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end);

NoirUI.ProcessParams = LPH_NO_VIRTUALIZE(function(self , Params , Fixed)
	Params = Params or {};
...
NoirUI.EnabledBlur = true;
NoirUI.BlurModuleParent = workspace.CurrentCamera;

NoirUI.GetCalculatePosition = LPH_NO_VIRTUALIZE(function(planePos, planeNormal, rayOrigin, rayDirection)
	local n = planeNormal;
...
NoirUI.CreateBlurModule = LPH_NO_VIRTUALIZE(function(self , Frame , Signal)
	if not NoirUI.EnabledBlur then
		return NoirUI:AddSignal(Instance.new('BindableEvent').Event:Connect(function() return "nl"; end));	
	end;

	local Part = Instance.new('Part',NoirUI.BlurModuleParent);
	...
	Part.CanQuery = false;
	Part.CollisionGroup = NoirUI.RandomString();
	...
	DepthOfField.Name = NoirUI.RandomString();

	Part.Name = NoirUI.RandomString();
	...
	local UpdateFunction = function()
		...
		if IsWindowActive and not NoirUI.Global3DRenderMode then

			NoirUI.PlayAnimate(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 1
			})

			NoirUI.PlayAnimate(Part,TweenInfo.new(0.1),{
				Transparency = 0.97,
				Size = Vector3.new(1, 1, 1) * 0.01;
			})

			Part.Parent = NoirUI.BlurModuleParent;
		else
			NoirUI.PlayAnimate(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 0
			})
	...
	local rbxsignal = NoirUI:AddSignal(CurrentCamera:GetPropertyChangedSignal('CFrame'):Connect(UpdateFunction))
	local loopThread = NoirUI:AddSignal(UserInputService.InputChanged:Connect(function(Input)
...
function NoirUI:CreateShadow(parent , RollingEffect)
	...
	if RollingEffect then
		r1 = NoirUI:RollingEffect(UIShadowSafe85);
		r2 = NoirUI:RollingEffect(UIShadowSafe65);
		r3 = NoirUI:RollingEffect(UIShadowSafe50);
		r4 = NoirUI:RollingEffect(UIShadowSafe45);
	end;
	...
	Shadow.Render = LPH_NO_VIRTUALIZE(function(self , value)
		...
		if value then
			NoirUI.PlayAnimate(UIShadowSafe85 , SlowyTween , {
				Transparency = 0.900
			})

			NoirUI.PlayAnimate(UIShadowSafe65 , SlowyTween , {
				Transparency = 0.900
			})

			NoirUI.PlayAnimate(UIShadowSafe50 , SlowyTween , {
				Transparency = 0.900
			})

			NoirUI.PlayAnimate(UIShadowSafe45 , SlowyTween , {
				Transparency = 0.900
			})
			...
			if RollingEffect then
				RollingEffectThread = task.spawn(function()
					...
					while true do task.wait(0.025)
						NoirUI.PlayAnimate(r1 , SlowyTween , {
							Rotation = r1.Rotation + level
						});

						NoirUI.PlayAnimate(r2 , SlowyTween , {
							Rotation = r2.Rotation + level
						});

						NoirUI.PlayAnimate(r3 , SlowyTween , {
							Rotation = r3.Rotation + level
						});

						NoirUI.PlayAnimate(r4 , SlowyTween , {
...
function NoirUI:CreateOptionWindow(Frame: Frame , Zindex)
	Zindex = Zindex or 9;

	local Window = {
		Signal = NoirUI:CreateSignal(false),
	};
	...
	local shadow = NoirUI:CreateShadow(OptionHandler);

	OptionHandler.Name = NoirUI.RandomString();
	OptionHandler.Parent = NoirUI.ScreenGui
	...
	NoirUI:AddSignal(UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
		NoirUI.PlayAnimate(OptionHandler , SlowyTween , {
			Size = UDim2.new(0, 220, 0, UIListLayout.AbsoluteContentSize.Y - 1)
		})
	end)));

	NoirUI:AddSignal(OptionHandler:GetPropertyChangedSignal('BackgroundTransparency'):Connect(LPH_NO_VIRTUALIZE(function()
		if OptionHandler.BackgroundTransparency > 0.9 then
			...
			if NoirUI.Global3DRenderMode then
				OptionHandler.Parent = NoirUI.GlobalSurfaceGui;
			else
				OptionHandler.Parent = NoirUI.ScreenGui;
			end;
		end
	...
	local SetPosition = LPH_NO_VIRTUALIZE(function()
		if NoirUI:MoreThanHalfY(Frame.AbsolutePosition.Y + 65) then
	...
	Window.SetRender = LPH_NO_VIRTUALIZE(function(value)
		...
		if value then
			...
			NoirUI.PlayAnimate(UIStroke , SlowyTween , {
				Transparency = 0.650
			})
	...
	local Payback = NoirUI:RegisiterItem(OptionHandler , Window.Signal);
...
function NoirUI:CreateColorPicker(HandleFrame: Frame)
	...
	local Shadow = NoirUI:CreateShadow(ColorPickerHandler);

	ColorPickerHandler.Name = NoirUI.RandomString();
	ColorPickerHandler.Parent = NoirUI.ScreenGui
	...
	NoirUI:AddSignal(ColorPickerHandler:GetPropertyChangedSignal('BackgroundTransparency'):Connect(LPH_NO_VIRTUALIZE(function()
		if ColorPickerHandler.BackgroundTransparency > 0.9 then
			...
			if NoirUI.Global3DRenderMode then
				ColorPickerHandler.Parent = NoirUI.GlobalSurfaceGui;
			else
				ColorPickerHandler.Parent = NoirUI.ScreenGui;
			end;
		end;
	...
	SaViMap.Name = NoirUI.RandomString();
	...
	SaViMap.Image = NoirUI.ImageColorMapping -- UNSAFE IMAGE

	UICorner_2.CornerRadius = UDim.new(0, 5)
	UICorner_2.Parent = SaViMap

	ColorZoneSelection.Name = NoirUI.RandomString();
	...
	ColorMap.Name = NoirUI.RandomString();
	...
	ColorMapSelection.Name = NoirUI.RandomString();
	...
	RGBLabel.Name = NoirUI.RandomString();
	...
	ColorPickerLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
		if value then
			...
			NoirUI.PlayAnimate(ColorPickerHandler,SlowyTween , {
				BackgroundTransparency = 0.035
			})
			...
			NoirUI.PlayAnimate(SaViMap,SlowyTween , {
				BackgroundTransparency = 0,
				ImageTransparency = 0
			})

			NoirUI.PlayAnimate(UIStroke_2,SlowyTween , {
				Transparency = 0
			})

			NoirUI.PlayAnimate(ColorMap,SlowyTween , {
				BackgroundTransparency = 0
			})

			NoirUI.PlayAnimate(UIStroke_3,SlowyTween , {
				Transparency = 0
			})

			NoirUI.PlayAnimate(RGBLabel,SlowyTween , {
	...
	function ColorPickerLib:Update()
		...
		NoirUI.PlayAnimate(ColorZoneSelection,ManualTween,{
			Position = UDim2.fromScale(ColorPickerLib.S , 1 - ColorPickerLib.V)
		});

		NoirUI.PlayAnimate(SaViMap,ManualTween,{
			BackgroundColor3 = Color3.fromHSV(ColorPickerLib.H , 1 , 1)
		});

		NoirUI.PlayAnimate(ColorMapSelection,ManualTween,{
	...
	NoirUI:AddSignal(ColorPickerHandler.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			ColorPickerLib.IsHold = true;
		end;
	end));

	NoirUI:AddSignal(ColorPickerHandler.InputEnded:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			ColorPickerLib.IsHold = false;
		end;
	end));

	NoirUI:AddSignal(ColorMap.InputBegan:Connect(LPH_NO_VIRTUALIZE(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			ColorPickerLib.IsHold = true;
	...
	NoirUI:AddSignal(SaViMap.InputBegan:Connect(LPH_NO_VIRTUALIZE(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			ColorPickerLib.IsHold = true;
...
for i,v in next , NoirUI.KeyEnum do
	NoirUI.EnumReverse[v] = i;
end;

function NoirUI:KeyCodeToStr(K: Enum.KeyCode)
	if typeof(K) == 'string' then
		if NoirUI.KeyEnum[K] then
			return NoirUI.KeyEnum[K];
		end;

		return K;
	end;

	return (NoirUI.KeyEnum[K.Name] or K.Name);
end;

function NoirUI:StrToKeyCode(str: string)
	if NoirUI.EnumReverse[str] then
		return Enum.KeyCode[NoirUI.EnumReverse[str]];
	end;

	return Enum.KeyCode[str];
end;

function NoirUI:RegisiterHandler(Handler: Frame , Signal)
	local handle = {};
	local ZINdex = Handler.ZIndex;

	function handle:AddToggle(Config)
		Config = NoirUI:ProcessParams(Config , {
			Default = false,
			Flag = nil,
		...
		Toggle.Name = NoirUI.RandomString();
		...
		Circle.Name = NoirUI.RandomString();
		...
		ToggleLib.SetUI = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(Toggle,SlowyTween,{
					BackgroundTransparency = 0,
					BackgroundColor3 = NoirUI.AccentColor
				})

				NoirUI.PlayAnimate(Circle,SlowyTween,{
		...
		NoirUI:CreateInput(Toggle , LPH_NO_VIRTUALIZE(function()
		...
		if Config.Flag then
			NoirUI.Flags[Config.Flag] = ToggleLib;
		end;
	...
	function handle:AddSlider(Config)
		...
		Slider.Name = NoirUI.RandomString();
		...
		ValueFrame.Name = NoirUI.RandomString();
		...
		ValueLabel.Name = NoirUI.RandomString();
		...
		SlideMain.Name = NoirUI.RandomString();
		...
		SlideFrame.Name = NoirUI.RandomString();
		...
		SlideMoving.Name = NoirUI.RandomString();
		SlideMoving.Parent = SlideFrame
		SlideMoving.BackgroundColor3 = NoirUI.AccentColor
		...
		ValueLabel.FocusLost:Connect(LPH_NO_VIRTUALIZE(function()
			local OutVal = NoirUI:ParseInput(ValueLabel.Text , true);
			if OutVal then
				local rx = math.clamp(OutVal , Config.Min , Config.Max);
				local Value = NoirUI.Rounding(rx,Config.Rounding);
		...
		SliderLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(ValueFrame,SlowyTween,{
					BackgroundTransparency = 0,
				...
				NoirUI.PlayAnimate(ValueLabel,SlowyTween,{
					TextTransparency = 0.350
				});

				NoirUI.PlayAnimate(SlideFrame,SlowyTween,{
					BackgroundTransparency = 0
				});

				NoirUI.PlayAnimate(SlideMoving,SlowyTween,{
					BackgroundTransparency = 0,
					Size = UDim2.new(SliderLib.GetSize(), 0, 1, 0)
				});

				NoirUI.PlayAnimate(Frame,SlowyTween,{
					BackgroundTransparency = 0
				});
			else
				NoirUI.PlayAnimate(ValueFrame,SlowyTween,{
		...
		local Update = function(Input)
			...
			local Value = NoirUI.Rounding(Main,Config.Rounding);
			...
			SlideMain.InputEnded:Connect(LPH_NO_VIRTUALIZE(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not NoirUI:IsMouseOverFrame(SlideMain) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		if Config.Flag then
			NoirUI.Flags[Config.Flag] = SliderLib;
		end;
	...
	function handle:AddOption(GearIcon)
		...
		Option.Name = NoirUI.RandomString();
		...
		Icon.FontFace = NoirUI.BuiltInBold
		...
		local Window = NoirUI:CreateOptionWindow(Option , ZINdex + 13);
		local reciveSignal;

		Window.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(Icon , SlowyTween , {
					TextTransparency = 0.400
				})
			else
				NoirUI.PlayAnimate(Icon , SlowyTween , {
		...
		local bthg = NoirUI:CreateInput(Option , LPH_NO_VIRTUALIZE(function()
			...
			reciveSignal = UserInputService.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if not NoirUI:IsMouseOverFrame(Window.Root) and not NoirUI:IsMouseOverFrame(Option) then
						if reciveSignal then
							reciveSignal:Disconnect();
							reciveSignal = nil;	
						end;
		...
		NoirUI:AddSignal(bthg.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(Option , SlowyTween , {
		...
		NoirUI:AddSignal(bthg.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(Option , SlowyTween , {
	...
	function handle:AddColorPicker(Config)
		...
		ColorPicker.Name = NoirUI.RandomString();
		...
		local BackendM = NoirUI:CreateColorPicker(ColorPicker);
		...
		local signal;
		NoirUI:CreateInput(ColorPicker , LPH_NO_VIRTUALIZE(function()
			if signal then
				signal:Disconnect();
				signal = nil;
			end;
			...
			signal = UserInputService.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if not NoirUI:IsMouseOverFrame(ColorPicker) and not NoirUI:IsMouseOverFrame(BackendM.Root) then
						if signal then
							signal:Disconnect();
							signal = nil;
						end;
		...
		ColorPickerLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(ColorPicker , SlowyTween , {
					BackgroundTransparency = 0
				})
				...
				NoirUI.PlayAnimate(ImageLabel , SlowyTween , {
		...
		if Config.Flag then
			NoirUI.Flags[Config.Flag] = ColorPickerLib;
		end;
	...
	function handle:AddKeybind(Config)
		...
		Keybind.Name = NoirUI.RandomString();
		...
		ValueLabel.ZIndex = ZINdex + 14
		ValueLabel.Font = Enum.Font.GothamMedium
		ValueLabel.Text = NoirUI:KeyCodeToStr(Config.Default or "None")
		ValueLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
		ValueLabel.TextSize = 10.000
		...
		KeybindLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(Keybind,SlowyTween, {
					BackgroundTransparency = 0
				})
		...
		NoirUI:CreateInput(Keybind , function()
			if IsBinding then
				return;
			end;
			...
			ValueLabel.Text = NoirUI:KeyCodeToStr(KeyName);
		...
		function KeybindLib:SetValue(v)
			Config.Default = v;
			ValueLabel.Text = NoirUI:KeyCodeToStr(v);
			KeybindLib:Update();
			Config.Callback(Config.Default);
		end;

		if Config.Flag then
			NoirUI.Flags[Config.Flag] = KeybindLib;
		end;
	...
	function handle:AddTextInput(Config)
		...
		TextInput.Name = NoirUI.RandomString();
		...
		TextBoxLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(TextInput , SlowyTween ,{
					BackgroundTransparency = 0
				})	
				...
				NoirUI.PlayAnimate(TextBox , SlowyTween ,{
					TextTransparency = 0.350
				})	
			else
				NoirUI.PlayAnimate(TextInput , SlowyTween ,{
		...
		NoirUI:AddSignal(TextBox:GetPropertyChangedSignal('Text'):Connect(LPH_NO_VIRTUALIZE(function()
			local valout = NoirUI:ParseInput(TextBox.Text , Config.Numeric);
		...
		if Config.Flag then
			NoirUI.Flags[Config.Flag] = TextBoxLib;
		end;
	...
	function handle:AddDropdown(Config)
		...
		Config.Default = NoirUI.ProcessDropdown(Config.Default);
		...
		Dropdown.Name = NoirUI.RandomString();
		...
		DropdownIcon.Name = NoirUI.RandomString();
		...
		DropdownIcon.FontFace = NoirUI.BuiltInBold
		...
		BasedLabel.Name = NoirUI.RandomString();
		...
		BasedLabel.Text = NoirUI.ParseDropdown(Config.Default);
		...
		NoirUI:AddSignal(Dropdown.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(BasedLabel , SlowyTween , {
				TextTransparency = 0.200
			})
		end)));

		NoirUI:AddSignal(Dropdown.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(BasedLabel , SlowyTween , {
		...
		local DropdownLib = {
			OpenSignal = NoirUI:CreateSignal(false),
			Signals = {},
			Refuse = {},
		};
		...
		DropdownLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(Dropdown , SlowyTween , {
					BackgroundTransparency = 0
				});

				NoirUI.PlayAnimate(DropdownIcon , SlowyTween , {
		...
		do
			...
			local UIListLayout = Instance.new("UIListLayout")
			local Shadow = NoirUI:CreateShadow(DropdownHandler);

			DropdownHandler.Name = NoirUI.RandomString();
			DropdownHandler.Parent = NoirUI.ScreenGui;
			...
			NoirUI:AddSignal(DropdownHandler:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				if DropdownHandler.BackgroundTransparency > 0.9 then
					...
					if NoirUI.Global3DRenderMode then
						DropdownHandler.Parent = NoirUI.GlobalSurfaceGui;
					else
						DropdownHandler.Parent = NoirUI.ScreenGui;
					end;
				end;
			...
			DropdownScrollFrame.Name = NoirUI.RandomString();
		...
		local SecureSignal;
		NoirUI:CreateInput(Dropdown , LPH_NO_VIRTUALIZE(function()
			...
			NoirUI.IsMosueOverOtherFrame = true;

			SecureSignal = UserInputService.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if not NoirUI:IsMouseOverFrame(DropdownLib.BlockRoot) and not NoirUI:IsMouseOverFrame(Dropdown) then
						if SecureSignal then
							SecureSignal:Disconnect();
							SecureSignal = nil;
						end;
		...
		function DropdownLib:Generate()
			...
			for i,Value in next , Config.Values do
				...
				ItemFrame.Name = NoirUI.RandomString();
				...
				ItemLabel.Name = NoirUI.RandomString();
				...
				if Config.Multi then
					...
					local VisiblewOfMult = LPH_NO_VIRTUALIZE(function()
						if DropdownLib.IsMatch(Value) then
							NoirUI.PlayAnimate(ItemLabel , VSlowTween , {
								TextTransparency = 0.200,
								Position = UDim2.new(0, 30, 0, 4)
							})

							NoirUI.PlayAnimate(Icon , vs , {
								TextTransparency = 0.250
							})
					...
					local DefaultVisible = LPH_NO_VIRTUALIZE(function()
						if DropdownLib.IsMatch(Value) then
							NoirUI.PlayAnimate(ItemLabel , SlowyTween , {
								TextTransparency = 0.200
							})
				...
				table.insert(DropdownLib.Signals,ItemFrame.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
					NoirUI.PlayAnimate(ItemFrame , SlowyTween , {
				...
				table.insert(DropdownLib.Signals , DropdownLib.OpenSignal:Connect(LPH_NO_VIRTUALIZE(function(val)
					if val then
						...
						if MIcon then
							NoirUI.PlayAnimate(MIcon , SlowyTween , {
								TextTransparency = 1
							})
						end;
					end;
				...
				if Config.Multi then
					local _,bth_signal = NoirUI:CreateInput(ItemFrame , LPH_NO_VIRTUALIZE(function()
		...
		if Config.Flag then
			NoirUI.Flags[Config.Flag] = DropdownLib;
		end;
...
NoirUI.ProcessDropdown = LPH_NO_VIRTUALIZE(function(value)
...
NoirUI.ParseDropdown = LPH_NO_VIRTUALIZE(function(value)
...
function NoirUI:ParseInput(Value , Numeric)
	if not Value then
...
function NoirUI:CreateToolTips(Container: Frame , Name: string , Content: string)
	local Tooltips = Instance.new("Frame")
	local UICorner = Instance.new("UICorner")
	local UIStroke = Instance.new("UIStroke")
	local TooltipName = Instance.new("TextLabel")
	local TooltipContent = Instance.new("TextLabel")
	local Shadow = NoirUI:CreateShadow(Tooltips);

	Tooltips.Name = NoirUI.RandomString();
	...
	TooltipName.Name = NoirUI.RandomString();
	...
	TooltipContent.Name = NoirUI.RandomString();
	...
	ToolTip.Update = LPH_NO_VIRTUALIZE(function()
		...
		NoirUI.PlayAnimate(Tooltips,SlowyTween , {
			Size = UDim2.new(0,MaxX,0,MaxY)
		})
	end)

	NoirUI:AddSignal(Tooltips:GetPropertyChangedSignal('BackgroundTransparency'):Connect(LPH_NO_VIRTUALIZE(function()
		if Tooltips.BackgroundTransparency > 0.9 then
			...
			if NoirUI.Global3DRenderMode then
				Tooltips.Parent = NoirUI.GlobalSurfaceGui;
			else
				Tooltips.Parent = NoirUI.ScreenGui;
			end;
		end
	end)));
	...
	ToolTip.SetRender = LPH_NO_VIRTUALIZE(function(value)
		if value then
			...
			NoirUI.PlayAnimate(TooltipName , SlowyTween , {
				TextTransparency = 0
			})

			NoirUI.PlayAnimate(TooltipContent , SlowyTween , {
	...
	local DelayThread;
	NoirUI:AddSignal(Container.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
		if DelayThread then
	...
	NoirUI:AddSignal(Container.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
		if DelayThread then
			task.cancel(DelayThread);
			DelayThread = nil;
		end;
...
function NoirUI:RegisiterItem(Frame: Frame , Signel)
	local idx = {};
	...
	function idx:AddLabel(Name: string,Warp: boolean)
		...
		BasedFrame.Name = NoirUI.RandomString();
		...
		NoirUI:AddQuery(BasedFrame , Name);
		...
		LineFrame.Name = NoirUI.RandomString();
		...
		BasedHandler.Name = NoirUI.RandomString();
		...
		local UpdateWarp = LPH_NO_VIRTUALIZE(function()
			...
			NoirUI.PlayAnimate(BasedFrame , SlowyTween , {
		...
		local handle = NoirUI:RegisiterHandler(BasedHandler , Signel);
		...
		handle.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				...
				NoirUI.PlayAnimate(LineFrame , SlowyTween , {
		...
		NoirUI:AddSignal(BasedFrame.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(BasedFrame , SlowyTween , {
		...
		NoirUI:AddSignal(BasedFrame.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(BasedFrame , SlowyTween , {
				BackgroundTransparency = 1
			});
		...
		function handle:ToolTip(Content: string)
			handle.ToolTip = NoirUI:CreateToolTips(BasedFrame , Name , Content);
	...
	function idx:AddButton(Config)
		...
		NoirUI:AddQuery(ButtonFrame , Config.Name);

		ButtonFrame.Name = NoirUI.RandomString();
		...
		local bth = NoirUI:CreateInput(ButtonFrame , LPH_NO_VIRTUALIZE(function()
			Config.Callback();
		end));

		NoirUI:AddSignal(bth.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(ButtonFrame , SlowyTween , {
				BackgroundTransparency = 0.35
			});
		end)))

		NoirUI:AddSignal(bth.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(ButtonFrame , SlowyTween , {
				BackgroundTransparency = 1
			});
		end)))
		...
		if Config.ToolTip then
			Button.ToolTip = NoirUI:CreateToolTips(ButtonFrame , Config.Name , Config.ToolTip);
		end;
	...
	function idx:AddUserFrame(Name : string , Profile: string , Expires : string)
		...
		UserFrame.Name = NoirUI.RandomString();
		...
		UserLabel.Name = NoirUI.RandomString();
		...
		LogoImage.Name = NoirUI.RandomString();
		...
		UserStatusLabel.Name = NoirUI.RandomString();
		...
		UserFrameItem.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				NoirUI.PlayAnimate(UserLabel,SlowyTween,{
					TextTransparency = 0.200
				})
				...
				NoirUI.PlayAnimate(LogoImage,SlowyTween,{
					ImageTransparency = 0
				})

				NoirUI.PlayAnimate(UserStatusLabel,SlowyTween,{
					TextTransparency = 0.200
				})
			else
				NoirUI.PlayAnimate(UserLabel,SlowyTween,{
...
function NoirUI:CreateWindow(Config)
	...
	local Window = {
		...
		Signal = NoirUI:CreateSignal(true),
		Tabs = {},
		CurrentTab = 1,
	...
	NoirUI.GlobalLogo = Window.Logo;

	local Logging = NoirUI:CreateLogger();
	...
	WindowFrame.Name = NoirUI.RandomString();
	WindowFrame.Parent = NoirUI.ScreenGui;
	...
	local renderParentWindow = LPH_NO_VIRTUALIZE(function()
		if Window.__3DRender then
			if WindowFrame.BackgroundTransparency > 0.9 then
				...
				NoirUI.PlayAnimate(WindowFrame,VSlowTween , {
					Position = UDim2.fromScale(0.5,0.5);
				});
	...
	NoirUI:AddSignal(WindowFrame:GetPropertyChangedSignal('BackgroundTransparency'):Connect(renderParentWindow))

	Window.SetRender = LPH_NO_VIRTUALIZE(function(self , value)
		if value then
			NoirUI.PlayAnimate(WindowFrame , SlowyTween , {
				BackgroundTransparency = (NoirUI.EnabledBlur and 0.055) or 0.0255,
				Size = Window.Size
			})
			...
			NoirUI.PlayAnimate(WindowName , SlowyTween , {
				TextTransparency = 0
			})

			NoirUI.PlayAnimate(WindowContent , SlowyTween , {
				TextTransparency = 0.650
			})
			...
			NoirUI.PlayAnimate(AccountProfile , SlowyTween , {
				ImageTransparency = 0
			})

			NoirUI.PlayAnimate(AccountName , SlowyTween , {
				TextTransparency = 0
			})

			NoirUI.PlayAnimate(ExpireLabel , SlowyTween , {
				TextTransparency = 0.650
			})

			NoirUI.PlayAnimate(LineFrame_2 , SlowyTween , {
				BackgroundTransparency = 0.650
			})

			NoirUI.PlayAnimate(UserSettingButton , SlowyTween , {
				TextTransparency = 0.5
			})

			NoirUI.PlayAnimate(RightMenuFrame , SlowyTween , {
			...
			NoirUI.PlayAnimate(LineFrame_3 , SlowyTween , {
				BackgroundTransparency = 0.650
			})

			NoirUI.PlayAnimate(ConfigFrame , SlowyTween , {
				BackgroundTransparency = 0.750
			})
			...
			NoirUI.PlayAnimate(ConfigIcon , SlowyTween , {
				TextTransparency = 0.250
			})

			NoirUI.PlayAnimate(LineFrame_4 , SlowyTween , {
				BackgroundTransparency = 0.650
			})

			NoirUI.PlayAnimate(ConfigName , SlowyTween , {
				TextTransparency = 0.350
			})

			NoirUI.PlayAnimate(ConfigBthIcon , SlowyTween , {
				TextTransparency = 0.250
			})

			NoirUI.PlayAnimate(SearchIcon , SlowyTween , {
				TextTransparency = 0.250
			})

			NoirUI.PlayAnimate(SearchBox , SlowyTween , {
				TextTransparency = 0.350
			})
	...
	Window.Shadow = NoirUI:CreateShadow(WindowFrame);
	Window.Shadow:Render(false);

	task.delay(0.25,function()
		WindowFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
		Window:SetRender(true);
		NoirUI:AddSignal(Window.Signal:Connect(LPH_NO_VIRTUALIZE(function(...)
			Window:SetRender(...);
		end)))
	end)

	if NoirUI.EnabledBlur then
		NoirUI:CreateBlurModule(WindowFrame,Window.Signal);
	end;
	...
	do
		...
		NoirUI.Drag(Frame , WindowFrame , 0.15)
	end

	UICorner.Parent = WindowFrame

	LeftMenuFrame.Name = NoirUI.RandomString();
	...
	HeadFrame.Name = NoirUI.RandomString();
	...
	LogoImage.ImageColor3 = NoirUI.IconColor

	UICorner_2.CornerRadius = UDim.new(0, 7)
	UICorner_2.Parent = LogoImage

	WindowName.Name = NoirUI.RandomString();
	...
	WindowContent.Name = NoirUI.RandomString();
	...
	LeftScrollingFrame.Name = NoirUI.RandomString();
	...
	BottomFrame.Name = NoirUI.RandomString();
	...
	AccountProfile.Name = NoirUI.RandomString();
	...
	AccountProfile.Image = NoirUI.UserProfile or ""

	UICorner_3.CornerRadius = UDim.new(1, 0)
	UICorner_3.Parent = AccountProfile

	AccountName.Name = NoirUI.RandomString();
	...
	ExpireLabel.Name = NoirUI.RandomString();
	...
	LineFrame_2.Name = NoirUI.RandomString();
	...
	UserSettingButton.Name = NoirUI.RandomString();
	...
	UserSettingButton.FontFace = NoirUI.BuiltInBold
	...
	NoirUI:AddSignal(BottomFrame.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
		NoirUI.PlayAnimate(UserSettingButton,SlowyTween , {
			TextTransparency = 0.25
		})		
	end)))

	NoirUI:AddSignal(BottomFrame.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
		NoirUI.PlayAnimate(UserSettingButton,SlowyTween , {
			TextTransparency = 0.5
		})		
	end)))

	RightMenuFrame.Name = NoirUI.RandomString();
	...
	RightHeader.Name = NoirUI.RandomString();
	...
	LineFrame_3.Name = NoirUI.RandomString();
	...
	ConfigFrame.Name = NoirUI.RandomString();
	...
	ConfigIcon.Name = NoirUI.RandomString();
	...
	ConfigIcon.ZIndex = 9
	ConfigIcon.FontFace = NoirUI.BuiltInBold
	...
	LineFrame_4.Name = NoirUI.RandomString();
	...
	ConfigName.Name = NoirUI.RandomString();
	...
	ConfigBthIcon.Name = NoirUI.RandomString();
	...
	ConfigBthIcon.FontFace = NoirUI.BuiltInBold
	...
	SearchFrame.Name = NoirUI.RandomString();
	...
	SearchIcon.Name = NoirUI.RandomString();
	...
	SearchIcon.FontFace = NoirUI.BuiltInBold
	...
	SearchBox.Name = NoirUI.RandomString();
	...
	TabContainer.Name = NoirUI.RandomString();
	...
	do
		...
		local Input = NoirUI:CreateInput(SearchIcon , LPH_NO_VIRTUALIZE(function()
			Window.Searching = not Window.Searching;

			if Window.Searching then
				NoirUI.PlayAnimate(SearchFrame , VSlowTween , {
				...
				NoirUI.PlayAnimate(SearchBox , VSlowTween , {
					TextTransparency = 0.350
				})
			else
				NoirUI.PlayAnimate(SearchFrame , VSlowTween , {
		...
		NoirUI:AddSignal(SearchBox:GetPropertyChangedSignal('Text'):Connect(LPH_NO_VIRTUALIZE(function()
			if not SearchBox.Text:byte() then
				for i,v in next , NoirUI.NameRegisitry do
					v.Root.Visible = true;
				end;
		...
		NoirUI:AddSignal(Input.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(SearchIcon , SlowyTween , {
				TextTransparency = 0.25
			})
		end)))

		NoirUI:AddSignal(Input.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			if Window.Searching then
				NoirUI.PlayAnimate(SearchIcon , SlowyTween , {
					TextTransparency = 0.25
				})
	...
	if Window.Enable3DRenderer then
		...
		SurfaceGui.Parent = NoirUI.ScreenGui;
		...
		NoirUI.GlobalSurfaceGui = SurfaceGui;
		...
		Window.Load3DBlock = LPH_NO_VIRTUALIZE(function()
			if not Window.Signal:GetValue() then
				...
				if OnScreen then
					NoirUI.PlayAnimate(Part,VSlowTween , {
			...
			Part.Parent = NoirUI.BlurModuleParent or workspace;
	...
	function Window:AddTabLabel(Name: string)
		...
		TabLabel.Name = NoirUI.RandomString()
		...
		local SetRender = LPH_NO_VIRTUALIZE(function(val)
			if val then
				NoirUI.PlayAnimate(TabLabel , SlowyTween,{
					TextTransparency = 0.500
				})
			else
				NoirUI.PlayAnimate(TabLabel , SlowyTween,{
	...
	function Window:AddTab(Config)
		...
		TabButton.Name = NoirUI.RandomString();
		...
		TabIcon.Name = NoirUI.RandomString();
		...
		TabIcon.ZIndex = 9
		TabIcon.FontFace = NoirUI.BuiltInBold
		TabIcon.Text = Config.Icon;
		TabIcon.TextColor3 = NoirUI.AccentColor
		TabIcon.TextSize = 16.000
		TabIcon.TextWrapped = true

		TabContentLabel.Name = NoirUI.RandomString();
		...
		TabFrame.Name = NoirUI.RandomString();
		...
		LeftScroll.Name = NoirUI.RandomString();
		...
		RightScroll.Name = NoirUI.RandomString();
		...
		if Config.Type == "Single" then
			...
			NoirUI:AddSignal(UIListLayout_2:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
				RightScroll.CanvasSize = UDim2.fromOffset(0,UIListLayout_2.AbsoluteContentSize.Y + 1)
			end)))
		end;

		NoirUI:AddSignal(TabIcon:GetPropertyChangedSignal('TextTransparency'):Connect(LPH_NO_VIRTUALIZE(function()
		...
		Tab.SetValue = LPH_NO_VIRTUALIZE(function(value)
			...
			if value then
				NoirUI.PlayAnimate(TabButton , SlowyTween , {
					BackgroundTransparency = 0.500
				})

				NoirUI.PlayAnimate(TabIcon , SlowyTween , {
					TextTransparency = 0,
					TextColor3 = NoirUI.AccentColor
				})

				NoirUI.PlayAnimate(TabContentLabel , SlowyTween , {
					TextTransparency = 0
				})
			else
				NoirUI.PlayAnimate(TabButton , SlowyTween , {
		...
		local over = NoirUI:CreateInput(TabButton,LPH_NO_VIRTUALIZE(function()
		...
		NoirUI:AddSignal(over.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			if Window.Tabs[Window.CurrentTab] == Tab then
				NoirUI.PlayAnimate(TabButton , SlowyTween , {
					BackgroundTransparency = 0.500
				})
		...
		NoirUI:AddSignal(over.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			if Window.Tabs[Window.CurrentTab] == Tab then
				NoirUI.PlayAnimate(TabButton , SlowyTween , {
					BackgroundTransparency = 0.500
				})
		...
		function Tab:AddSection(Config)
			...
			SectionFrame.Name = NoirUI.RandomString();
			...
			SectionLabel.Name = NoirUI.RandomString();
			...
			SectionHandler.Name = NoirUI.RandomString();
			...
			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
				...
				if UIListLayout.AbsoluteContentSize.Y <= 1 then
					NoirUI.PlayAnimate(SectionFrame , VSlowTween , {
			...
			local Section = NoirUI:RegisiterItem(SectionHandler , Tab.Signal);

			Section.SetRender = LPH_NO_VIRTUALIZE(function(value)
				if value then
					NoirUI.PlayAnimate(SectionLabel,SlowyTween,{
						TextTransparency = 0.500
					})

					NoirUI.PlayAnimate(SectionHandler,SlowyTween,{
						BackgroundTransparency = 0.500
					})
	...
	function Window:_InitConfig()
		local ConfigSignal = NoirUI:CreateSignal(false);
		local ConfigLib = {
			Signals = {},
		};
		...
		local shadow = NoirUI:CreateShadow(ConfigMenu);
		...
		ConfigLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
			if value then
				...
				NoirUI.PlayAnimate(ConfigMenu , SlowyTween , {
					BackgroundTransparency = 0.035,
		...
		NoirUI:AddSignal(ConfigMenu:GetPropertyChangedSignal('BackgroundTransparency'):Connect(LPH_NO_VIRTUALIZE(function()
			if ConfigMenu.BackgroundTransparency > 0.9 then
				...
				if NoirUI.Global3DRenderMode then
					ConfigMenu.Parent = NoirUI.GlobalSurfaceGui;
				else
					ConfigMenu.Parent = NoirUI.ScreenGui;
				end;
			end
		end)))

		ConfigMenu.Name = NoirUI.RandomString();
		ConfigMenu.Parent = NoirUI.ScreenGui;
		...
		InputFrame.Name = NoirUI.RandomString();
		...
		LoadConfig.Name = NoirUI.RandomString();
		...
		OpenButton.Name = NoirUI.RandomString();
		...
		function ConfigLib:GetData(performance)
			...
			local cd = 0;
			for Flag,v in next , NoirUI.Flags do
			...
			return NoirUI.Base64Encode(Encryption.new(HttpService:JSONEncode(ikc)));
		end;

		function ConfigLib:LoadData(data)
			local coded = HttpService:JSONDecode(Encryption.reverse(NoirUI.Base64Decode(data)));

			for i,v in next , coded do
				if v.Idx then
					if NoirUI.Flags[v.Idx] then
						task.spawn(function()
							NoirUI.Flags[v.Idx]:SetValue(v.Value)
						end)
					end;
				end;
			end;
		...
		function ConfigLib:RefreshConfig()
			...
			for i,ConfigNameStr in next , ConfigList do
				...
				ConfigItemFrame.Name = NoirUI.RandomString();
				...
				DeleteConfig.Name = NoirUI.RandomString();
				...
				Icon_2.Name = NoirUI.RandomString();
				...
				Icon_2.ZIndex = 153
				Icon_2.FontFace = NoirUI.BuiltInBold
				...
				local Render = LPH_NO_VIRTUALIZE(function(rst)
					if rst then
						NoirUI.PlayAnimate(ConfigItemFrame,SlowyTween,{
							BackgroundTransparency = 0
						})
						...
						NoirUI.PlayAnimate(Icon_2,SlowyTween,{
							TextTransparency = 0.400
						})
				...
				local deleter,signal = NoirUI:CreateInput(DeleteConfig,function()
				...
				local _,load_signal = NoirUI:CreateInput(LoadConfig,function()
		...
		local hover_write = NoirUI:CreateInput(ConfigIcon,function()
		...
		NoirUI:AddSignal(hover_write.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(ConfigIcon,SlowyTween,{
				TextTransparency = 0.1
			})
		end)));

		NoirUI:AddSignal(hover_write.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(ConfigIcon,SlowyTween,{
				TextTransparency = 0.25
			})
		end)));
		...
		local mv = NoirUI:CreateInput(LoadConfig , function()
		...
		NoirUI:AddSignal(mv.MouseEnter:Connect(function()
			NoirUI.PlayAnimate(Icon , SlowyTween , {
				TextTransparency = 0.1
			})
		end))

		NoirUI:AddSignal(mv.MouseLeave:Connect(function()
			NoirUI.PlayAnimate(Icon , SlowyTween , {
				TextTransparency = 0.35
			})
		end))
		...
		OpenButton.MouseButton1Click:Connect(LPH_NO_VIRTUALIZE(function()
			...
			ConfigLib.UnsafeThread = UserInputService.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if not NoirUI:IsMouseOverFrame(ConfigMenu) then
						if ConfigLib.UnsafeThread then
							ConfigLib.UnsafeThread:Disconnect();
							ConfigLib.UnsafeThread = nil;
						end;
	...
	local UserSettings = NoirUI:CreateOptionWindow(BottomFrame , BottomFrame.ZIndex + 13);
	local reciveSignal;
	NoirUI:CreateInput(BottomFrame , LPH_NO_VIRTUALIZE(function()
		if reciveSignal then
			reciveSignal:Disconnect();
			reciveSignal = nil;	
		end;
		...
		reciveSignal = UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if not NoirUI:IsMouseOverFrame(UserSettings.Root) and not NoirUI:IsMouseOverFrame(BottomFrame) and not NoirUI.IsMosueOverOtherFrame then
					if reciveSignal then
						reciveSignal:Disconnect();
						reciveSignal = nil;	
					end;
	...
	NoirUI:AddSignal(UserInputService.InputBegan:Connect(LPH_NO_VIRTUALIZE(function(value,ISTYPING)
	...
	function Window:Watermark()
		if NoirUI.__WatermarkCache then
			return NoirUI.__WatermarkCache;
		end;
		...
		local UICorner = Instance.new("UICorner")
		local UIListLayout = Instance.new("UIListLayout")
		local Shadow = NoirUI:CreateShadow(Watermark);

		Watermark.Name = NoirUI.RandomString();
		Watermark.Parent = NoirUI.ScreenGui
		...
		UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
			NoirUI.PlayAnimate(Watermark , SlowyTween , {
				Size = UDim2.new(0, UIListLayout.AbsoluteContentSize.X + 5, 0, 30)
			})
		end));

		NoirUI.__WatermarkCache = Watermark_lb;
		...
		function Watermark_lb:AddBlock(IconStr , Name)
			...
			Content.Name = NoirUI.RandomString();
			...
			Icon.FontFace = NoirUI.BuiltInBold;
			Icon.Text = IconStr
			Icon.TextColor3 = NoirUI.AccentColor
			Icon.TextSize = 18.000
			Icon.TextTransparency = 0.250
			Icon.TextWrapped = true
			...
			InnerBlock.Update = LPH_NO_VIRTUALIZE(function(value)
				...
				if InnerBlock.Visible then
					NoirUI.PlayAnimate(Frame,VSlowTween,{
			...
			InnerBlock.SetRender = LPH_NO_VIRTUALIZE(function(value)
				if value and InnerBlock.Visible then
					NoirUI.PlayAnimate(Content,SlowyTween , {
						TextTransparency = 0.200
					})
			...
			function InnerBlock:Input(func)
				local c,s = NoirUI:CreateInput(Frame,func);
...
function NoirUI:CreateNotification()
	if NoirUI.__Notification_Cache then
		return NoirUI.__Notification_Cache;
	end;
	...
	Notification.Name = NoirUI.RandomString();
	Notification.Parent = NoirUI.ScreenGui;
	...
	NoirUI.__Notification_Cache = Notifier;

	function Notifier.new(Config)
		Config = NoirUI:ProcessParams(Config , {
			Title = "Notification",
			Content = "Hello World!",
			Logo = NoirUI.GlobalLogo or "rbxasset://textures/ui/VerifiedBadgeNameIcon.png",
			Duration = 5,
		});

		if NoirUI.__WatermarkCache then
			NoirUI.PlayAnimate(Notification,SlowyTween , {
		...
		local shadow = NoirUI:CreateShadow(NotifyFrame , true);

		ContainerFrame.Name = NoirUI.RandomString();
		...
		NotifyFrame.Name = NoirUI.RandomString();
		...
		NotifyName.Name = NoirUI.RandomString();
		...
		NotifyContent.Name = NoirUI.RandomString();
		...
		shadow:Render(true)
		NoirUI.PlayAnimate(NotifyFrame , VSlowTween , {
		...
		task.delay(Config.Duration or 5 , LPH_NO_VIRTUALIZE(function()
			...
			NoirUI.PlayAnimate(NotifyFrame , SlowyTween , {
				BackgroundTransparency = 1
			})
			...
			NoirUI.PlayAnimate(NotifyName , SlowyTween , {
				TextTransparency = 1
			})

			NoirUI.PlayAnimate(NotifyContent , SlowyTween , {
				TextTransparency = 1
			})

			task.wait(0.125);

			NoirUI.PlayAnimate(ContainerFrame , SlowyTween , {
...
function NoirUI:CreateLogger()
	...
	Log.Name = NoirUI.RandomString();
	Log.Parent = NoirUI.ScreenGui
	Log.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	Log.BackgroundTransparency = 1.000
	Log.BorderColor3 = Color3.fromRGB(0, 0, 0)
	Log.BorderSizePixel = 0
	Log.Position = UDim2.new(0, 25, 0, 5 + math.abs(NoirUI.ScreenGui.AbsolutePosition.Y))
	...
	function Logging.new(IconStr: string , Message: string , Duration: number)
		...
		local UICorner_2 = Instance.new("UICorner")
		local Icon = Instance.new("TextLabel")
		local Shadow = NoirUI:CreateShadow(LogFrame , true);

		LogFrame.Name = NoirUI.RandomString();
		LogFrame.Parent = Log
		...
		LogContent.Name = NoirUI.RandomString();
		...
		Line.Name = NoirUI.RandomString();
		Line.Parent = LogFrame
		Line.AnchorPoint = Vector2.new(0, 0.5)
		Line.BackgroundColor3 = NoirUI.AccentColor
		...
		NoirUI.PlayAnimate(LogFrame , SlowyTween , {
		...
		task.delay(0.15,LPH_NO_VIRTUALIZE(function()
			...
			NoirUI.PlayAnimate(LogContent , SlowyTween , {
				TextTransparency = 0.25
			});

			NoirUI.PlayAnimate(Line , SlowyTween , {
				BackgroundTransparency = 0
			});
...
function NoirUI:CreateIndicator()
	...
	IndicatorFrame.Name = NoirUI.RandomString();
	IndicatorFrame.Parent = NoirUI.ScreenGui;
	...
	function Indicators.new(Config)
		...
		local Icon = Instance.new("TextLabel")
		local Content = Instance.new("TextLabel")
		local Shadow = NoirUI:CreateShadow(IndicatorItem);

		IndicatorItem.Name = NoirUI.RandomString();
		...
		Indicator.Update = LPH_NO_VIRTUALIZE(function()
			...
			NoirUI.PlayAnimate(IndicatorItem , SlowyTween , {
		...
		Indicator.SetRender = LPH_NO_VIRTUALIZE(function(self , value)
			...
			if value then
				...
				NoirUI.PlayAnimate(Icon , VSlowTween , {
					TextTransparency = 0.250,
					TextColor3 = Indicators.Color[Indicator.CurrentColor]
				});

				NoirUI.PlayAnimate(Content , VSlowTween , {
					TextTransparency = 0.2,
					TextColor3 = Indicators.Color[Indicator.CurrentColor]
				});
...
function NoirUI:Unload()
	if not NoirUI.UnloadEnabled then
		return;	
	end;

	NoirUI.ScreenGui:Destroy();

	for i,v in next , NoirUI.GlobalSignals do
