--[[
    ...
    License: MIT
    Github: https://github.com/4lpaca-pin/CompKiller (Original)
    Original: compkiller.net

        Version: 2.6 (NoirUI Edition)
    - Improved Performance Mode
    	Noir:OptimizeMode(< boolean >)
  	- Added Auto Color Logo
		Noir:CustomIconHighlight()
]]
...
local Noir = {
	Version = '2.6',
	Logo = "rbxthumb://type=Asset&id=94916110511437&w=420&h=420",
	Windows = {},
	Scale = {
...
function Noir:OptimizeMode(v)
	Noir.PerformanceMode = v;
end;

function Noir:IsStudio()
	return RunService:IsStudio()	
end;

function Noir:CustomIconHighlight()
	Noir.CustomHighlightMode = true;
end;

function Noir:_SetNilP(Ins: Instance , Parent: Instance)
	Noir.WindowsNil = Noir.WindowsNil or {};
	Noir.NilFolder = Noir.NilFolder or Instance.new('Folder');

	if not Noir.WindowsNil[Ins] then
		local win = Noir:_GetWindowFromElement(Ins);

		Noir.WindowsNil[Ins] = win;
	end;

	Ins.Parent = Parent or Noir.NilFolder;
end;

function Noir:SetAllText(flags : {[string] : string})
	if not flags then -- reset to default
		for i,v in next , Noir.Flags do
			if v.SetText then
				v:SetText(nil);
			end;
		end;
	...
	for i,v in next , flags do
		if Noir.Flags[i] and Noir.Flags[i].SetText then
			Noir.Flags[i]:SetText(v);
		end;
	end;
end;

function Noir:_GetIcon(name : string , font_aws) : string
	if Noir.SecureMode then
		local AssetId;

		if font_aws then
			AssetId = Noir.FontAwesome[name] or name;
		else
			AssetId = Noir.Lucide['lucide-'..tostring(name)] or Noir.Lucide[name] or Noir.Lucide[tostring(name)] or Noir.FontAwesome[name] or name;
		end;

		if AssetId and AssetId ~= nil then
			local asset = Noir:CacheImage(AssetId);
	...
	if font_aws then
		return Noir.FontAwesome[name] or name;
	end;

	return Noir.Lucide['lucide-'..tostring(name)] or Noir.Lucide[name] or Noir.Lucide[tostring(name)] or Noir.FontAwesome[name] or name;
end;

function Noir:_RandomString() : string
	return "NR="..string.char(math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102));	
end;

function Noir:_IsMouseOverFrame(Frame : Frame) : boolean
	if not Frame then
		return;
	end;
...
function Noir:_Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Noir:_Animation(Self: Instance , Info: TweenInfo , Property :{[K] : V})
...
function Noir:_Input(Frame : Frame , Callback : () -> ()) : TextButton
...
function Noir:GetCalculatePosition(planePos: number, planeNormal: number, rayOrigin: number, rayDirection: number) : number
	local n = planeNormal;
...
function Noir:_Blur(element : Frame , WindowRemote) : RBXScriptSignal
	if Noir.SecureMode and not Noir.SecurityConfig.BlurEnabled then
		return game.Changed:Connect(function() end);
	end;

	local Part = Instance.new('Part',Noir.ArcylicParent);
	...
	Part.CanQuery = false;
	Part.CollisionGroup = Noir:_RandomString();
	...
	DepthOfField.Name = Noir:_RandomString();

	Part.Name = Noir:_RandomString();
	...
	local UpdateFunction = function()
		...
		if IsWindowActive then
			...
			Noir:_Animation(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 1
			})

			Noir:_Animation(Part,TweenInfo.new(0.1),{
				Transparency = 0.97,
		...
		if IsWindowActive then
			...
			local pos0 = Noir:GetCalculatePosition(planeOrigin, planeNormal, ray0.Origin, ray0.Direction);
			local pos1 = Noir:GetCalculatePosition(planeOrigin, planeNormal, ray1.Origin, ray1.Direction);
...
function Noir:_AddDragBlacklist(Frame: Frame)
	local IsAdded = false;
	local BASE_TIME = 0.01;

	local SET_BLACKLIST = function(value)
		local index = table.find(Noir.DragBlacklist , Frame);

		if value and not Noir.IS_DRAG_MOVE then
			if not index then
				table.insert(Noir.DragBlacklist,Frame);
			end;
		else
			if index then
				table.remove(Noir.DragBlacklist,index);
			end;
		end;
	end;

	Frame.InputBegan:Connect(function(input)
		if Noir:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(true)
		end;
	end);
	...
	UserInputService.InputChanged:Connect(function()
		if not Noir:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(false);
		end
	end);
end;

function Noir:_GetWindowFromElement(Element)
	if Noir.WindowsNil[Element] then
		return Noir.WindowsNil[Element];
	end;

	for i,v : ScreenGui in next , Noir.Windows do
		if v and Element:IsDescendantOf(v) then
			return v;
		end;
	end;

	for Frame,Window in next , Noir.WindowsNil do
...
function Noir.__SIGNAL(default)
...
function Noir:_Hover(Frame: Frame , OnHover: () -> any?, Release: () -> any?)
	Frame.MouseEnter:Connect(OnHover);

	Frame.MouseLeave:Connect(Release);
end;

function Noir.__CONFIG(config , default)
	config = config or {};
...
function Noir:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local updateInput = function(input)
		...
		Noir:_Animation(MoveFrame,Tween,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Noir.DragBlacklist <= 0 then 
			dragToggle = true
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					dragToggle = false;
					Noir.IS_DRAG_MOVE = false;
				end
			end)
		end

		if not Noir.IsDrage and dragToggle then
			Noir.LastDrag = tick();
		end;

		Noir.IaDrag = dragToggle;
	end)

	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #Noir.DragBlacklist <= 0 then
			if dragToggle then
				Noir.IS_DRAG_MOVE = true;
				updateInput(input)
			else
				Noir.IS_DRAG_MOVE = false;
			end
		else
			if #Noir.DragBlacklist > 0 then
				dragToggle = false
				Noir.IS_DRAG_MOVE = false;
			end
		end
...
function Noir:_IsMobile()
	return UserInputService.TouchEnabled;
end;

function Noir:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	if Name == "Toggle" then
		...
		Toggle.Name = Noir:_RandomString()
		Toggle.Parent = LinkValues
		Toggle.BackgroundColor3 = Noir.Colors.DropColor
		...
		table.insert(Noir.Elements.DropColor , {
			Element = Toggle,
		...
		UIStroke.Color = Noir.Colors.HighStrokeColor
		UIStroke.Parent = Toggle

		table.insert(Noir.Elements.HighStrokeColor,{
			Element = UIStroke,
			Property = "Color"
		})

		ToggleValue.Name = Noir:_RandomString()
		ToggleValue.Parent = Toggle
		ToggleValue.AnchorPoint = Vector2.new(0.5, 0.5)
		ToggleValue.BackgroundColor3 = Noir.Colors.SwitchColor
		...
		local ToggleElement = function(bool,noChange)
			...
			if bool then
				...
				Noir:_Animation(ToggleValue,rep.Tween,{
					Position = UDim2.new(0.75, 0, 0.5, 0)
				})

				Noir:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Noir.Colors.Toggle
				})
			else
				...
				Noir:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Noir.Colors.DropColor
				})
			end;
		end;

		local Input = Noir:_Input(Toggle);

		Noir:_Hover(Input , function()
			if not Default then
				Noir:_Animation(ToggleValue,rep.Tween,{
		...
		local ToggleUI = function(bool)
			if bool then
				...
				Noir:_Animation(UIStroke,rep.Tween,{
					Transparency = 0
				})
			else
	...
	elseif Name == "ColorPicker" then
		...
		ColorPicker.Name = Noir:_RandomString()
		...
		ColorFrame.Name = Noir:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				Noir:_Animation(ColorFrame,TweenInfo.new(0.15),{
					BackgroundTransparency = 0,
				})

				Noir:_Animation(UIStroke,TweenInfo.new(0.15),{
					Transparency = 0,
				})
			else
				Noir:_Animation(ColorFrame,TweenInfo.new(0.15),{
		...
		Noir:_Hover(ColorPicker, function()
			if Signal:GetValue() then
				Noir:_Animation(UIScale,TweenInfo.new(0.35),{
					Scale = 1.2
				})
			end;
		end , function()
	...
	elseif Name == "Keybind" then
		...
		Keybind.Name = Noir:_RandomString()
		Keybind.Parent = LinkValues
		Keybind.BackgroundColor3 = Noir.Colors.DropColor
		...
		TextLabel.TextColor3 = Noir.Colors.SwitchColor
		TextLabel.TextSize = 12.000
		TextLabel.TextTransparency = 0.200

		table.insert(Noir.Elements.SwitchColor,{
			Element = TextLabel,
			Property = "TextColor3"
		});
		...
		local Update = function()
			...
			Noir:_Animation(Keybind,TweenInfo.new(0.1),{
		...
		local ToggleUI = function(bool)
			if bool then
				Noir:_Animation(Keybind,rep.Tween,{
				...
				Noir:_Animation(TextLabel,rep.Tween,{
	...
	elseif Name == "Helper" then
		...
		InfoButton.Name = Noir:_RandomString()
		...
		InfoButton.Image = Noir:CacheImage("rbxassetid://10723415903")
		InfoButton.ImageTransparency = 0.500

		UICorner.CornerRadius = UDim.new(1, 0)
		UICorner.Parent = InfoButton

		BlockText.Name = Noir:_RandomString()
		BlockText.Parent = InfoButton
		BlockText.AnchorPoint = Vector2.new(0, 0)
		BlockText.BackgroundColor3 = Noir.Colors.BlockColor

		table.insert(Noir.Elements.BlockColor , {
			Element = BlockText,
			Property = "BackgroundColor3"
		});
		...
		BlockText.Text = " "
		BlockText.TextColor3 = Noir.Colors.SwitchColor
		...
		UIStroke.Color = Noir.Colors.StrokeColor
		UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		UIStroke.Parent = BlockText

		table.insert(Noir.Elements.StrokeColor,{
			Element = UIStroke,
			Property = "Color"
		});
		...
		Signal:Connect(function(bool)
			if bool then
				Noir:_Animation(InfoButton,TweenInfo.new(0.15),{
		...
		Noir:_Hover(InfoButton, function()
			if Signal:GetValue() then
				Noir:_Animation(InfoButton,TweenInfo.new(0.15),{
	...
	elseif Name == "Option" then
		...
		OptionButton.Name = Noir:_RandomString()
		...
		OptionButton.Image = Noir:CacheImage("rbxassetid://14007344336")
		...
		Signal:Connect(function(bool)
			if bool then
				Noir:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.500
				})
			else
				Noir:_Animation(OptionButton,TweenInfo.new(0.15),{
		...
		Noir:_Hover(OptionButton, function()
			if Signal:GetValue() then
				Noir:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.1
				})
...
function Noir:_CreateBlock(Signal)
	...
	if Noir:_IsMobile() then
		Noir:_AddDragBlacklist(GlobalBlock);
	end;

	GlobalBlock.Name = Noir:_RandomString()
	...
	LinkValues.Name = Noir:_RandomString()
	...
	BlockLine.Name = Noir:_RandomString()
	BlockLine.Parent = GlobalBlock
	BlockLine.AnchorPoint = Vector2.new(0.5, 1)
	BlockLine.BackgroundColor3 = Noir.Colors.LineColor
	...
	table.insert(Noir.Elements.LineColor,{
		Element = BlockLine,
		Property = "BackgroundColor3"
	});
	...
	function rep:SetTextColor(Color)
		local oldIndex = table.find(Noir.Elements.SwitchColor , BlockText);

		table.remove(Noir.Elements.SwitchColor , oldIndex);

		BlockText.TextColor3 = Color;

		table.insert(Noir.Elements.Risky , {
			Element = BlockText,
			Property = 'TextColor3'
		});
	...
	function rep:SetTransparency(num)
		...
		Noir:_Animation(BlockText,TweenInfo.new(0.3),{
	...
	function rep:SetVisible(bool)
		if bool then
			Noir:_Animation(BlockText,rep.Tween,{
				TextTransparency = rep.TextTransparency
			});

			Noir:_Animation(BlockLine,rep.Tween,{
	...
	function rep:AddLink(Name , Default)
		return Noir:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal);
	end;

	return rep;
end;

Noir.Hash = function(str: string)
	...
	return "nr-"..tostring(math.round(hex + 15))..tostring(dh);
end;

function Noir:CacheImage(id: string) : string
	if not Noir.SecureMode or not id or not id:byte() then
		return id or "";
	end;

	assert(Noir.SecureMode , "please use Noir:Security(< string >) before cache image")
	assert(Noir.CacheDirectory , "please use Noir:Security(< string >) before cache image")
	...
	local Hash = Noir.Hash(id);

	local cache_path = string.format("%s/cache-%s.png" ,Noir.CacheDirectory , Hash);

	if isfile(cache_path) then
		return (getcustomasset or getsynasset or function() return ''; end)(cache_path);
	end;

	local imgSize = Noir.SecurityConfig.ImageScale;
...
function Noir:Security(directory: string,Config: SecurityConfig) -- Security Mode
	...
	Noir.SecureMode = true;

	Noir.SecurityConfig = Config or {};

	Noir.CacheDirectory = directory;
end;

function Noir:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	local Window = Noir:_GetWindowFromElement(Button);
	...
	ColorPickerWindow.Name = Noir:_RandomString()
	ColorPickerWindow.Parent = Window
	ColorPickerWindow.BackgroundColor3 = Noir.Colors.BlockBackground
	...
	table.insert(Noir.Elements.BlockBackground,{
		Element = ColorPickerWindow,
		Property = "BackgroundColor3"
	});

	Noir:_AddDragBlacklist(ColorPickerWindow)
	...
	ColorPickBox.Name = Noir:_RandomString()
	...
	ColorPickBox.Image = Noir:CacheImage("http://www.roblox.com/asset/?id=112554223509763");

	MouseMovement.Name = Noir:_RandomString()
	...
	MouseMovement.Image = Noir:CacheImage("rbxassetid://4805639000")
	...
	ColorRedGreenBlue.Name = Noir:_RandomString()
	...
	ColorRGBSlide.Name = Noir:_RandomString()
	...
	ColorOpc.Name = Noir:_RandomString()
	...
	ColorOptSlide.Name = Noir:_RandomString()
	...
	Left_2.Name = Noir:_RandomString()
	...
	Right_2.Name = Noir:_RandomString()
	...
	TransparentImage.Name = Noir:_RandomString()
	...
	TransparentImage.Image = Noir:CacheImage("rbxassetid://6198493000")
	...
	HexFrame.Name = Noir:_RandomString()
	HexFrame.Parent = ColorPickerWindow
	HexFrame.AnchorPoint = Vector2.new(0.5, 1)
	HexFrame.BackgroundColor3 = Noir.Colors.BlockColor
	...
	UIStroke_8.Color = Noir.Colors.HighStrokeColor
	...
	Noir:_AddPropertyEvent(ColorPickerWindow,function(v)
		ColorPickerWindow.Visible = v;

		if Noir.PerformanceMode then
			if ColorPickerWindow.Visible then
				Noir:_SetNilP(ColorPickerWindow , Window);
			else
				Noir:_SetNilP(ColorPickerWindow , nil);
			end;
		else
			Noir:_SetNilP(ColorPickerWindow , Window);
		end;
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Noir:_Animation(ColorPickerWindow,Tween2,{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 175, 0, 200)
			});

			Noir:_Animation(ColorPickerWindow,Tween,{
				Position = MainPosition,
			});

			Noir:_Animation(UIStroke_8,Tween,{
				Transparency = 0
			});

			Noir:_Animation(UIStroke_7,Tween,{
				Transparency = 0.5
			});

			Noir:_Animation(UIStroke_6,Tween,{
				Transparency = 0
			});

			Noir:_Animation(UIStroke_5,Tween,{
				Transparency = 0
			});

			Noir:_Animation(UIStroke_4,Tween,{
				Transparency = 0
			});

			Noir:_Animation(UIStroke_3,Tween,{
				Transparency = 0
			});

			Noir:_Animation(UIStroke_2,Tween,{
				Transparency = 0
			});

			Noir:_Animation(UIStroke,Tween,{
				Transparency = 0
			});

			Noir:_Animation(ColorPickBox,Tween,{
				BackgroundTransparency = 0,
				ImageTransparency = 0
			});

			Noir:_Animation(MouseMovement,Tween,{
				ImageTransparency = 0
			});

			Noir:_Animation(ColorOpc,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(TransparentImage,Tween,{
				ImageTransparency = 0
			});

			Noir:_Animation(Left,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(Left_2,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(Right,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(Right_2,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(ColorRedGreenBlue,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(HexFrame,Tween,{
				BackgroundTransparency = 0
			});

			Noir:_Animation(TextLabel,Tween,{
				TextTransparency = 0
			});
		else
			Noir:_Animation(UIStroke_8,Tween,{
	...
	function Args:Update()
		...
		Noir:_Animation(ColorPickBox,TweenInfo.new(0.2),{
			BackgroundColor3 = Color3.fromHSV(H , 1 , 1)
		});

		Noir:_Animation(ColorOpc,TweenInfo.new(0.2),{
			BackgroundColor3 = RealColor
		});

		Noir:_Animation(MouseMovement,TweenInfo.new(0.2),{
			Position = UDim2.fromScale(S , 1 - V)
		});

		Noir:_Animation(ColorOptSlide,TweenInfo.new(0.2),{
			Position = UDim2.new(Transparency ,0 , 0.5 ,0)
		});

		Noir:_Animation(ColorRGBSlide,TweenInfo.new(0.2),{
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Noir:_IsMouseOverFrame(ColorPickerWindow) then
				ToggleUI(false);
			end;
		end;
	end)
...
function Noir:_DrawKeybinds(Window: ScreenGui)
	if Noir.__KEYBINDS_CACHE then
		return Noir.__KEYBINDS_CACHE;
	end;
	...
	Keybinds.Name = Noir:_RandomString()
	Keybinds.Parent = Window
	Keybinds.BackgroundColor3 = Noir.Colors.BGDBColor;
	...
	table.insert(Noir.Elements.BGDBColor,{
		Element = Keybinds,
		Property = 'BackgroundColor3'
	});
	...
	IconFrame.Name = Noir:_RandomString()
	IconFrame.Parent = Keybinds
	IconFrame.AnchorPoint = Vector2.new(1, 0.5)
	IconFrame.BackgroundColor3 = Noir.Colors.BGDBColor
	...
	Frame.BackgroundColor3 = Noir.Colors.Highlight;
	...
	table.insert(Noir.Elements.Highlight,{
		Element = Frame,
		Property = 'BackgroundColor3'
	});

	Icon.Name = Noir:_RandomString()
	...
	Icon.ZIndex = 159
	Icon.Image = Noir:CacheImage("rbxassetid://10723416765");

	HeaderFrame.Name = Noir:_RandomString()
	...
	HeadLabel.Name = Noir:_RandomString()
	...
	HeadLabel.TextColor3 = Noir.Colors.SwitchColor
	...
	MainFrame.Name = Noir:_RandomString()
	...
	MovingFrame.Name = Noir:_RandomString()
	...
	Noir:Drag(MovingFrame,Keybinds,0.1);
	...
	Ref.THREAD = task.spawn(function()
		while true do task.wait()
			Noir:_Animation(MainFrame,TweenInfo.new(0.4),{
				Size = UDim2.new(1, 30, 1, UIListLayout.AbsoluteContentSize.Y + 1)
			});

			if UIListLayout.AbsoluteContentSize.Y > 1 then
				Noir:_Animation(IconFrame,TweenInfo.new(0.25),{
					BackgroundTransparency = 0.3
				})

				Noir:_Animation(Frame,TweenInfo.new(0.25),{
					BackgroundTransparency = 0
				})

				Noir:_Animation(HeadLabel,TweenInfo.new(0.25),{
					TextTransparency = 0
				})

				Noir:_Animation(Icon,TweenInfo.new(0.25),{
					ImageTransparency = 0
				});
				...
				Noir:_Animation(Keybinds,TweenInfo.new(0.25),{
			...
			if Noir.PerformanceMode then
				if Keybinds.Visible then
					Noir:_SetNilP(Keybinds , Window);
				else
					Noir:_SetNilP(Keybinds , nil);
				end;
			else
				Noir:_SetNilP(Keybinds , Window);
			end;
	...
	function Ref:AddFrame()
		...
		Keyholder.Name = Noir:_RandomString()
		Keyholder.BackgroundColor3 = Noir.Colors.BGDBColor
		...
		Label.TextColor3 = Noir.Colors.SwitchColor
		...
		Line.Name = Noir:_RandomString()
		Line.Parent = Keyholder
		Line.AnchorPoint = Vector2.new(1, 0.5)
		Line.BackgroundColor3 = Noir.Colors.BGDBColor
		...
		TypeLabel.Name = Noir:_RandomString()
		...
		TypeLabel.TextColor3 = Noir.Colors.SwitchColor
		...
		function frame_ref:SetVisible(v)
			if v then
				Noir:_Animation(Keyholder,TweenInfo.new(0.1),{
					BackgroundTransparency = 0.600,
					Size = UDim2.new(1, 0, 0, 28)
				});

				Noir:_Animation(Label,TweenInfo.new(0.15),{
					TextTransparency = 0.100
				});

				Noir:_Animation(Line,TweenInfo.new(0.15),{
					BackgroundTransparency = 0
				});

				Noir:_Animation(TypeLabel,TweenInfo.new(0.15),{
					TextTransparency = 0
				});
			else
				Noir:_Animation(Keyholder,TweenInfo.new(0.1),{
	...
	Noir.__KEYBINDS_CACHE = Ref;

	return Ref;
end;

function Noir:_KeybindHandler(Parent: Frame , ObjectType: string , ElementAPI: Toggle & Slider , Signal , Zindex: number , ElementCFG: Slider)
	local Window = Noir:_GetWindowFromElement(Parent);
	local KB_Signal = Noir.__SIGNAL(false);
	local SubIndex = math.random(40,100);
	local KeybindInd = Noir:_DrawKeybinds(Window);
	...
	KeybindHandler.Name = Noir:_RandomString()
	KeybindHandler.Parent = Window
	KeybindHandler.BackgroundColor3 = Noir.Colors.BlockBackground
	...
	ElementObjs.Name = Noir:_RandomString()
	...
	local refreshPF = function()
		if Noir.PerformanceMode then
			if KeybindHandler.Size.Y.Offset > 1 then
				Noir:_SetNilP(KeybindHandler , Window);
			else
				Noir:_SetNilP(KeybindHandler , nil);
			end;
		else
			Noir:_SetNilP(KeybindHandler , Window);
		end;
	...
	local ToggleUI = function(bool)
		if bool then
			...
			Noir:_Animation(KeybindHandler,TweenInfo.new(0.25),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 225, 0, UIListLayout.AbsoluteContentSize.Y + 5)
			});

			Noir:_Animation(UIStroke,TweenInfo.new(0.3),{
				Transparency = 0
			});
		else
			Noir:_Animation(KeybindHandler,TweenInfo.new(0.3),{
				BackgroundTransparency = 1,
	...
	local ElementAPIs = Noir:_LoadElement(ElementObjs , true , KB_Signal , true);
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.MouseButton2 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Noir:_IsMouseOverFrame(Parent) and not Noir:_IsMouseOverFrame(KeybindHandler) then
...
function Noir:_LoadOption(Value , TabSignal)
	...
	function Args:AddKeybind(Config: MiniKeybind)
		...
		Noir:_Input(Keybind.Root,function()
			if IsBinding then
				return;
			end;
	...
	function Args:AddHelper(Config: Helper)
		...
		Helper.UIStroke:GetPropertyChangedSignal('Transparency'):Connect(function()
			...
			if Noir.PerformanceMode then
				if Helper.Text.Visible then
					Noir:_SetNilP(Helper.Text , Window);
				else
					Noir:_SetNilP(Helper.Text , nil);
				end;
			else
				Noir:_SetNilP(Helper.Text , Window);
			end;
		...
		local Update = function()
			...
			Noir:_Animation(Helper.Text , TweenInfo.new(0.15), {
		...
		local Release = function()
			...
			Noir:_Animation(Helper.UIStroke,TweenInfo.new(0.15),{
				Transparency = 1
			});
		end;
		...
		Noir:_Hover(Button,  Hold, Release);
	...
	function Args:AddColorPicker(Config: MiniColorPicker)
		...
		local Button = Noir:_Input(ColorPicker);

		local ColorPicker = Noir:_AddColorPickerPanel(Button,function(color,opc)
	...
	function Args:AddOption()
		...
		local Signal = Noir.__SIGNAL(false);
		...
		local ToggleUI = function(bool)
			...
			if bool then
				...
				Noir:_Animation(ExtractElement , Tween , {
					Position = MainPosition,
					BackgroundTransparency = 0,
		...
		Noir:_AddPropertyEvent(ExtractElement,function(bool)
			ExtractElement.Visible = bool;

			if Noir.PerformanceMode then
				if ExtractElement.Visible then
					Noir:_SetNilP(ExtractElement , Window);
				else
					Noir:_SetNilP(ExtractElement , nil);
				end;
			else
				Noir:_SetNilP(ExtractElement , Window);
			end;
		...
		Noir:_AddDragBlacklist(ExtractElement);

		ExtractElement.Name = Noir:_RandomString()
		ExtractElement.Parent = Window
		ExtractElement.BackgroundColor3 = Noir.Colors.BlockBackground
		...
		Elements.Name = Noir:_RandomString()
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if Toggl and not Noir:_IsMouseOverFrame(ExtractElement) and not Noir:_IsMouseOverFrame(Element) then
					ToggleUI(false);
				end;
			end
		end)		

		return Noir:_LoadElement(Elements , true , Signal)
	end;

	return Args;
end;

function Noir:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	local Window = Noir:_GetWindowFromElement(BaseParent);
	...
	local UIListLayout = Instance.new("UIListLayout")
	local ToggleDb = Noir.__SIGNAL(false);
	local EventOut = Noir.__SIGNAL(0);

	DropdownWindow.Name = Noir:_RandomString()
	DropdownWindow.Parent = Window
	DropdownWindow.BackgroundColor3 = Noir.Colors.BlockBackground
	...
	Noir:_AddDragBlacklist(DropdownWindow);
	Noir:_AddPropertyEvent(DropdownWindow,function(v)
		DropdownWindow.Visible = v;

		if Noir.PerformanceMode then
			if DropdownWindow.Visible then
				Noir:_SetNilP(DropdownWindow , Window);
			else
				Noir:_SetNilP(DropdownWindow , nil);
			end;
		else
			Noir:_SetNilP(DropdownWindow , Window);
		end;
	...
	local DrawButton = function()
		...
		DropdownItem.Name = Noir:_RandomString()
	...
	local UpdateDropdown = function()
		...
		for i,v in next , Values do
			...
			table.insert(__signals , ToggleDb:Connect(function(bool)
				if bool then
					Noir:_Animation(bth.BlockText,TweenInfo.new(0.2),{
						TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
					});

					Noir:_Animation(bth.BlockLine,TweenInfo.new(0.2),{
						BackgroundTransparency = 0
					});
				else
					Noir:_Animation(bth.BlockText,TweenInfo.new(0.2),{
			...
			if IsMulti then
				...
				Noir:_Input(bth.DropdownItem,function()
				...
				Noir:_Input(bth.DropdownItem,function()
					if DataFrame then
						Noir:_Animation(DataFrame.BlockText,TweenInfo.new(0.2),{
							TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
						});
					end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Noir:_IsMouseOverFrame(DropdownWindow) then
				ToggleUI(false);
			end;
		end;
	end);
...
function Noir:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddToggle(Config : Toggle)
		...
		local Block = Noir:_CreateBlock(Signal);
		...
		if Config.Risky then
			Block:SetTextColor(Noir.Colors.Risky);
		end;
		...
		Args.Link = Noir:_LoadOption(Block);
		...
		if not DisableStackKeybind then
			local AutoKeybind = Noir:_KeybindHandler(Block.Root , "Toggle" , Args , Signal , Zindex , Config);
	...
	function Args:AddButton(Config: Button)
		...
		if Noir:_IsMobile() then
			Noir:_AddDragBlacklist(Button);
		end;

		Button.Name = Noir:_RandomString()
		...
		Noir:_Hover(Frame,function()
			if Signal:GetValue() then
				Noir:_Animation(Frame,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				})
			end;
		end,function()
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Noir:_Animation(BlockLine, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.500
				});

				Noir:_Animation(Frame, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.1
				});

				Noir:_Animation(UIStroke, TweenInfo.new(0.35),{
					Transparency = 0
				});

				Noir:_Animation(TextLabel, TweenInfo.new(0.35),{
	...
	function Args:AddSlider(Config: Slider)
		...
		Noir:_AddDragBlacklist(Slider);

		Slider.Name = Noir:_RandomString()
		...
		SliderBar.Name = Noir:_RandomString()
		SliderBar.Parent = Slider
		SliderBar.AnchorPoint = Vector2.new(0.5, 1)
		SliderBar.BackgroundColor3 = Noir.Colors.DropColor
		...
		SliderInput.Name = Noir:_RandomString()
		SliderInput.Parent = SliderBar
		SliderInput.AnchorPoint = Vector2.new(0, 0.5)
		SliderInput.BackgroundColor3 = Noir.Colors.Highlight
		...
		Frame.BackgroundColor3 = Noir.Colors.SwitchColor
		Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Frame.BorderSizePixel = 0
		Frame.Position = UDim2.new(1, 5, 0.5, 0)
		...
		ValueText.Name = Noir:_RandomString()
		...
		ValueText.TextColor3 = Noir.Colors.SwitchColor
		...
		Noir:_Hover(SliderBar,function()
			if Signal:GetValue() then
				Noir:_Animation(ValueText,TweenInfo.new(0.2),{
		...
		local Update = function(Input)
			...
			local Value = Noir:_Rounding(Main,Config.Round);
			...
			SliderBar.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Noir:_IsMouseOverFrame(SliderBar) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		function Args:SetValue(Value)
			...
			Noir:_Animation(SliderInput, TweenInfo.new(0.35),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Noir:_Animation(ValueText,Tween,{
					TextTransparency = 0.750
				})

				Noir:_Animation(Frame,Tween,{
					BackgroundTransparency = 0
				})

				Noir:_Animation(SliderInput,Tween,{
					BackgroundTransparency = 0
				})
				...
				Noir:_Animation(SliderBar,Tween,{
					BackgroundTransparency = 0
				})

				Noir:_Animation(BlockLine,Tween,{
					BackgroundTransparency = 0.5
				})

				Noir:_Animation(BlockText,Tween,{
		...
		if not DisableStackKeybind then
			local AutoKeybind = Noir:_KeybindHandler(Slider , "Number" , Args , Signal , Zindex , Config);
	...
	function Args:AddParagraph(Config: Paragraph) -- request by Neptune
		...
		if Noir:_IsMobile() then
			Noir:_AddDragBlacklist(Paragraph);
		end;

		Paragraph.Name = Noir:_RandomString()
		...
		DescriptionText.Name = Noir:_RandomString()
		...
		DescriptionText.TextColor3 = Noir.Colors.SwitchColor
		...
		local UpdateScale = function()
			...
			if not DescriptionText.Text:byte() then
				...
				Noir:_Animation(Paragraph,TweenInfo.new(0.15),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Noir:_Animation(BlockText,TweenInfo.new(0.2),{
					TextTransparency = 0.300
				});

				Noir:_Animation(DescriptionText,TweenInfo.new(0.2),{
					TextTransparency = 0.500
				});

				Noir:_Animation(BlockLine,TweenInfo.new(0.2),{
					BackgroundTransparency = 0.500
				});
			else
				Noir:_Animation(BlockText,TweenInfo.new(0.2),{
	...
	function Args:AddTextBox(Config: TextBoxConfig)
		...
		if Noir:_IsMobile() then
			Noir:_AddDragBlacklist(TextBox);
		end;

		TextBox.Name = Noir:_RandomString()
		...
		LinkValues.AnchorPoint = Vector2.new(1, 0.540000021)
		LinkValues.BackgroundColor3 = Noir.Colors.DropColor
		LinkValues.BorderColor3 = Color3.fromRGB(0, 0, 0)
		LinkValues.BorderSizePixel = 0
		LinkValues.Position = UDim2.new(1, -12, 0.5, 0)
		...
		TextBox_2.TextColor3 = Noir.Colors.SwitchColor
		...
		local Update = function()
			...
			local xp = pcall(function()
				Noir:_Animation(LinkValues,TweenInfo.new(0.25),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Noir:_Animation(LinkValues,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
	...
	function Args:AddDropdown(Config : Dropdown)
		...
		Dropdown.Name = Noir:_RandomString()
		...
		ValueItems.Name = Noir:_RandomString()
		ValueItems.Parent = Dropdown
		ValueItems.AnchorPoint = Vector2.new(0.5, 1)
		ValueItems.BackgroundColor3 = Noir.Colors.DropColor
		...
		MainButton.Name = Noir:_RandomString()
		...
		MainButton.Image = Noir:CacheImage("rbxassetid://109535175596957")

		Noir:_Hover(ValueItems,function()
			Noir:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = Noir.Colors.MouseEnter
			});
		end,function()
			Noir:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = Noir.Colors.DropColor
		...
		local repi;
		local Button = Noir:_Input(ValueItems);

		repi = Noir:_LoadDropdown(Button,function(value)
			Config.Default = value;
		...
		repi.EventOut:Connect(function(v)
			if v then
				Noir:_Animation(MainButton,TweenInfo.new(0.2),{
					Rotation = -180
				})
			else
				Noir:_Animation(MainButton,TweenInfo.new(0.2),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Noir:_Animation(ValueItems,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
				...
				Noir:_Animation(ValueText,TweenInfo.new(0.32),{
					TextTransparency = 0
				});
		...
		Args.Link = Noir:_LoadOption({
			AddLink = function(self ,Name , Default)
				return Noir:_AddLinkValue(Name , Default , LinkValues , LinkValues , {
...
function Noir:GetTheme()
	return Noir.Colors;
end;

function Noir:SetTheme(name)
	local themes = {
        Default = {
            Name = "Default",
            Highlight = Color3.fromRGB(139, 92, 246),
        ...
        Lunar = {
            ...
            Toggle = Color3.fromRGB(229, 231, 235),
            Risky = Color3.fromRGB(255, 200, 50),
...
function Noir:RefreshCurrentColor()
	for i,v in next , Noir.Elements.Highlight do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.Highlight;
		end;
	end;

	for i,v in next , Noir.Elements do
		if v.Element and v.Property and v.Element:GetAttribute('Enabled') then
			v.Element[v.Property] = Noir.Colors.Highlight;
		end;
	end;

	for i,v in next , Noir.Elements.Risky do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.Risky;
		end;
	end;

	for i,v in next , Noir.Elements.BlockColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.BlockColor;
		end;
	end;

	for i,v in next , Noir.Elements.BGDBColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.BGDBColor;
		end;
	end;

	for i,v in next , Noir.Elements.StrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.StrokeColor;
		end;
	end;

	for i,v in next , Noir.Elements.SwitchColor do
		if v.Element and v.Property and v.Element[v.Property] ~= Noir.Colors.MouseEnter then
			v.Element[v.Property] = Noir.Colors.SwitchColor;
		end;
	end;

	for i,v in next , Noir.Elements.BlockBackground do
		if v.Element and v.Property and v.Element[v.Property] then
			v.Element[v.Property] = Noir.Colors.BlockBackground;
		end;
	end;

	for i,v in next , Noir.Elements.DropColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.DropColor;
		end;
	end;

	for i,v in next , Noir.Elements.LineColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.LineColor;
		end;
	end;

	for i,v in next , Noir.Elements.HighStrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Noir.Colors.HighStrokeColor;
		end;
	end;
end;

function Noir:ChangeHighlightColor(NewColor: Color3)
	local H,S,V = NewColor:ToHSV();

	Noir.Colors.Highlight = NewColor;
	Noir.Colors.Toggle = Color3.fromHSV(H,S,V - 0.2);
...
function Noir.new(Config : Window)

	if not Config.Scale then
		if Noir:_IsMobile() then
			Config.Scale = Noir.Scale.Mobile;
		else
			Config.Scale = Noir.Scale.Window;
		end;
	end;
	...
	Config = Noir.__CONFIG(Config , {
		...
		Keybind = "Insert",
		Logo = Noir.Logo;
		Scale = Noir.Scale.Window,
		TextSize = 15
	});

	local TabHover = Noir.__SIGNAL(false);
	local WindowOpen = Noir.__SIGNAL(true);
	local WindowArgs = {
		SelectedTab = nil,
		Tabs = {},
		LastTab = nil,
	...
	Noir.ProtectGui(CompKiller);

	WindowArgs.Root = CompKiller;

	table.insert(Noir.Windows , CompKiller);

	MainFrame.Active = true;
	MainFrame.Name = Noir:_RandomString()
	MainFrame.Parent = CompKiller
	MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	MainFrame.BackgroundColor3 = Noir.Colors.BGDBColor
	...
	MainFrame.Size = Noir.Scale.Window
	MainFrame.ZIndex = 4
	...
	Noir:_Animation(MainFrame,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
		Size = Config.Scale
	});
	...
	TabFrame.Name = Noir:_RandomString()
	TabFrame.Parent = MainFrame
	TabFrame.AnchorPoint = Vector2.new(1, 0)
	TabFrame.BackgroundColor3 = Noir.Colors.BGDBColor
	...
	LineFrame1.Name = Noir:_RandomString()
	LineFrame1.Parent = TabFrame
	LineFrame1.AnchorPoint = Vector2.new(1, 0)
	LineFrame1.BackgroundColor3 = Noir.Colors.BGDBColor
	...
	CompLogo.Name = Noir:_RandomString()
	...
	if Noir.CustomHighlightMode then
		CompLogo.ImageColor3 = Noir.Colors.Highlight;
	...
	WindowLabel.Name = Noir:_RandomString()
	...
	WindowLabel.TextColor3 = Noir.Colors.SwitchColor
	...
	TabButtons.Name = Noir:_RandomString()
	...
	SelectionFrame.Name = Noir:_RandomString()
	SelectionFrame.Parent = TabButtons
	SelectionFrame.AnchorPoint = Vector2.new(1, 0)
	SelectionFrame.BackgroundColor3 = Noir.Colors.Highlight
	...
	TabButtonScrollingFrame.Name = Noir:_RandomString()
	...
	Userinfo.Name = Noir:_RandomString()
	...
	do
		...
		Highlight.Name = Noir:_RandomString()
		...
		Userinfo.MouseEnter:Connect(function()
			Noir:_Animation(Highlight,TweenInfo.new(0.2),{
	...
	UserProfile.Name = Noir:_RandomString()
	...
	UserProfile.Image = Noir:CacheImage("rbxassetid://18518299306")

	UICorner_4.CornerRadius = UDim.new(1, 0)
	UICorner_4.Parent = UserProfile

	UserText.Name = Noir:_RandomString()
	...
	UserText.TextColor3 = Noir.Colors.SwitchColor
	...
	ExpireText.Name = Noir:_RandomString()
	...
	ExpireText.TextColor3 = Noir.Colors.SwitchColor
	...
	TabMainFrame.Name = Noir:_RandomString()
	...
	if Noir:_IsMobile() then
		Noir:_AddDragBlacklist(TabButtons);
	end;
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			Noir:_Animation(MainFrame,TweenInfo.new(0.2),{
				Size = Config.Scale
			})

			Noir:_Animation(TabButtonScrollingFrame,TweenInfo.new(0.35),{
				Position = UDim2.new(0.5, 0, 0.5, 0)
			})

			Noir:_Animation(CompLogo,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Noir:_Animation(WindowLabel,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Noir:_Animation(UserProfile,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Noir:_Animation(UserText,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Noir:_Animation(ExpireText,TweenInfo.new(0.2),{
			...
			Noir:_Animation(LineFrame1,TweenInfo.new(0.3),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 20, 1, 0)
			})

			Noir:_Animation(TabFrame,TweenInfo.new(0.2),{
			...
			Noir:_Animation(LineFrame1,TweenInfo.new(0.1),{
				BackgroundTransparency = 1,
			...
			Noir:_Animation(TabFrame,TweenInfo.new(0.1),{
				BackgroundTransparency = 1
			})
		end;
	end);
	...
	TabHover:Connect(function(value)
		...
		if value then
			Noir:_Animation(TabFrame , Style , {
				Size = UDim2.new(0, 185,1, 0)
			});

			Noir:_Animation(WindowLabel , Style , {
				Position = UDim2.new(0, 60,0, 17),
				TextTransparency = 0
			});

			Noir:_Animation(UserText , Style , {
				Position = UDim2.new(0, 55,0, 8),
				TextTransparency = 0.1
			});

			Noir:_Animation(ExpireText , Style , {
	...
	do
		...
		UserSettings.Name = Noir:_RandomString()
		UserSettings.Parent = CompKiller;
		UserSettings.BackgroundColor3 = Noir.Colors.BGDBColor;
		...
		SectionFrame.Name = Noir:_RandomString()
		SectionFrame.Parent = UserSettings
		SectionFrame.BackgroundColor3 = Noir.Colors.BGDBColor;
		...
		Header.Name = Noir:_RandomString()
		...
		HeaderText.Name = Noir:_RandomString()
		...
		ImageLabel.Image = Noir:CacheImage("rbxassetid://10747384394")
		...
		function WindowArgs.UserSettings:Create()
			...
			WindowArgs.UserSettings.Signal = Noir:_Blur(UserSettings,Signal);

			Noir:Drag(UserSettings , UserSettings, 0.15);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Noir:_Animation(UserSettings,TweenInfo.new(0.2),{
			...
			function WindowArgs.UserSettings:Window(Value)
				if Value then
					...
					Noir:_Animation(SectionFrame,TweenInfo.new(0.2),{
						BackgroundTransparency = 0,
					});

					Noir:_Animation(HeaderText,TweenInfo.new(0.2),{
						TextTransparency = 0,
					});

					Noir:_Animation(ImageLabel,TweenInfo.new(0.2),{
			...
			Noir:_Input(ImageLabel,function()
				WindowArgs.UserSettings:Window(false);
			end);

			WindowArgs.UserSettings:Window(false);

			return Noir:_LoadElement(SectionFrame , true , Signal);
		end;
	end;
	...
	function WindowArgs:DrawCategory(config : Category)
		...
		Category.Name = Noir:_RandomString()
		...
		if Noir:_IsMobile() then
			Noir:_AddDragBlacklist(Category);
		end;

		CategoryText.Name = Noir:_RandomString()
		...
		CategoryText.TextColor3 = Noir.Colors.SwitchColor
		...
		TabHover:Connect(function(bool)
			if bool then
				Noir:_Animation(CategoryText,Tween,{
					TextTransparency = 0.500
				});
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		TabConfig = Noir.__CONFIG(TabConfig,{
			Name = "Tab",
			Icon = "eye",
		});

		local Tween = TweenInfo.new(0.35,Enum.EasingStyle.Quint);
		local TabOpenSignal = Noir.__SIGNAL(false);
		...
		TabButton.Name = Noir:_RandomString()
		...
		if Noir:_IsMobile() then
			Noir:_AddDragBlacklist(TabButton);
		end;

		Icon.Name = Noir:_RandomString()
		Icon.Parent = TabButton
		Icon.AnchorPoint = Vector2.new(0, 0.5)
		Icon.BackgroundColor3 = Noir.Colors.Highlight
		Icon.BackgroundTransparency = 1.000
		Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Icon.BorderSizePixel = 0
		...
		Icon.ZIndex = 3
		Icon.Image = Noir:_GetIcon(TabConfig.Icon);
		Icon.ImageColor3 = Noir.Colors.Highlight
		...
		TabNameLabel.Name = Noir:_RandomString()
		...
		TabNameLabel.TextColor3 = Noir.Colors.SwitchColor
		...
		ContainerTab.Name = Noir:_RandomString()
		...
		Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
			...
			if Noir.PerformanceMode then
				if ContainerTab.Visible then
					Noir:_SetNilP(ContainerTab , TabMainFrame);
				else
					Noir:_SetNilP(ContainerTab , nil);
				end;
			else
				Noir:_SetNilP(ContainerTab , TabMainFrame);
			end;
		...
		local TabOpen = function(bool)
			if bool then
				...
				Noir:_Animation(Icon,Tween,{
					ImageTransparency = 0,
				});

				Noir:_Animation(TabNameLabel,Tween,{
					TextTransparency = 0
				});

				Noir:_Animation(Highlight,Tween,{
					BackgroundTransparency = 0.925
				});
		...
		Noir:_Hover(TabButton,function()
		...
		TabHover:Connect(function(bool)
			if bool then
				Noir:_Animation(TabButton,Tween,{
				...
				Noir:_Animation(UICorner,Tween,{
		...
		Noir:_Input(TabButton,function()
		...
		function TabArgs:DrawTab(TabConfig : TabConfig) -- Internal Tab
			...
			local InternalSignal = Noir.__SIGNAL(false);
			local Frame = Instance.new("Frame")
			local UICorner = Instance.new("UICorner")
			local UIStroke = Instance.new("UIStroke")
			local Highlight = Instance.new("Frame")
			...
			Frame.Parent = Top
			Frame.BackgroundColor3 = Noir.Colors.BlockColor
			...
			Highlight.BackgroundColor3 = Noir.Colors.Highlight
			...
			local ToggleUI = function(bool)
				...
				if bool then
					...
					Noir:_Animation(TextLabel,TweenInfo.new(0.2),{
						TextTransparency = 0
					})
				else
	...
	function WindowArgs:DrawConfig(Configuration : TabConfigManager , Internal)
		Configuration = Noir.__CONFIG(Configuration,{
			Name = "Config",
			Icon = "folder",
			Config = nil
		});
		...
		Icon.Size = UDim2.new(0, 15, 0, 15)
		Icon.ZIndex = 3
		Icon.Image = Noir:_GetIcon(Configuration.Icon);
		Icon.ImageColor3 = Noir.Colors.Highlight
		...
		TabConfig.Name = Noir:_RandomString()
		...
		ConfigList.Name = Noir:_RandomString()
		ConfigList.Parent = TabConfig
		ConfigList.AnchorPoint = Vector2.new(0.5, 0)
		ConfigList.BackgroundColor3 = Noir.Colors.BlockColor
		...
		SectionText.Name = Noir:_RandomString()
		...
		SectionText.TextColor3 = Noir.Colors.SwitchColor
		...
		SectionClose.Name = Noir:_RandomString()
		...
		SectionClose.Image = Noir:CacheImage("rbxassetid://109535175596957")
		...
		UIStroke_2.Color = Noir.Colors.StrokeColor
		...
		Header_2.Name = Noir:_RandomString()
		...
		SectionText_2.Name = Noir:_RandomString()
		...
		SectionText_2.TextColor3 = Noir.Colors.SwitchColor
		...
		SectionClose_2.Name = Noir:_RandomString()
		...
		SectionClose_2.Image = Noir:CacheImage("rbxassetid://109535175596957")
		...
		UIStroke_3.Color = Noir.Colors.StrokeColor
		...
		TextBox.Text = ""
		TextBox.TextColor3 = Noir.Colors.SwitchColor
		...
		Button.BackgroundColor3 = Noir.Colors.SwitchColor
		Button.BackgroundTransparency = 1.000
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		...
		Frame_2.BackgroundColor3 = Noir.Colors.Highlight
		...
		UIStroke_4.Color = Noir.Colors.StrokeColor
		...
		Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
			...
			if Noir.PerformanceMode then
				if TabConfig.Visible then
					Noir:_SetNilP(TabConfig , TabMainFrame);
				else
					Noir:_SetNilP(TabConfig , nil);
				end;
			else
				Noir:_SetNilP(TabConfig , TabMainFrame);
			end;
		...
		local TabOpen = function(bool)
			if bool then
				...
				Noir:_Animation(ConfigList,Tween,{
					BackgroundTransparency = 0,
				});

				Noir:_Animation(AddConfig,Tween,{
					BackgroundTransparency = 0,
				});
				...
				Noir:_Animation(SectionText,Tween,{
					TextTransparency = 0.5
				});
				...
				Noir:_Animation(SectionText_2,Tween,{
					TextTransparency = 0.5
				});

				Noir:_Animation(TextBox,Tween,{
					TextTransparency = 0
				});

				Noir:_Animation(SectionClose,Tween,{
					ImageTransparency = 0.5,
				});

				Noir:_Animation(SectionClose_2,Tween,{
					ImageTransparency = 0.5,
				});
			else
		...
		function TabArgs:_DrawConfig()
			...
			DelButton.Name = Noir:_RandomString()
			...
			DelButton.ZIndex = 14
			DelButton.Image = Noir:CacheImage("rbxassetid://10747362393")
			...
			ConfigBlock.Name = Noir:_RandomString()
			...
			if Noir:_IsMobile() then
				Noir:_AddDragBlacklist(ConfigBlock);
			end;

			ConfigText.Name = Noir:_RandomString()
			...
			ConfigText.TextColor3 = Noir.Colors.SwitchColor
			...
			SaveButton.Name = Noir:_RandomString()
			...
			Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
			Icon.ZIndex = 15
			Icon.Image = Noir:CacheImage("rbxassetid://10734941499")
			Icon.ImageTransparency = 1;

			LoadButton.Name = Noir:_RandomString()
			...
			TextLabel_2.TextColor3 = Noir.Colors.SwitchColor
			...
			Icon_2.Name = Noir:_RandomString()
			...
			Icon_2.ZIndex = 15
			Icon_2.Image = Noir:CacheImage("rbxassetid://10723344270")
			...
			AuthorText.Name = Noir:_RandomString()
			...
			AuthorText.TextColor3 = Noir.Colors.SwitchColor
			...
			function ConfigButton:SetInfo(Author , ConfigName)
				local R,G,B = tostring(math.floor(Noir.Colors.Highlight.R * 255)) , tostring(math.floor(Noir.Colors.Highlight.G * 255)) , tostring(math.floor(Noir.Colors.Highlight.B * 255));
			...
			function ConfigButton:Toggle(v)
				if v then
					Noir:_Animation(ConfigBlock,Tween,{
						BackgroundTransparency = 0
					});

					Noir:_Animation(LinkValues,Tween,{
						Position = UDim2.new(1, -12, 0.5, 0)
					});

					Noir:_Animation(ConfigText,Tween,{
						TextTransparency = 0.3,
					...
					Noir:_Animation(AuthorText,Tween,{
						TextTransparency = 0.5,
						Position = UDim2.new(0,AuthorText:GetAttribute('SPC'), 0.5, 0)
					});

					Noir:_Animation(Icon_2,Tween,{
						ImageTransparency = 0
					});
					...
					Noir:_Animation(TextLabel_2,Tween,{
						TextStrokeTransparency = 0.900,
						TextTransparency = 0
					});
				else
			...
			Noir:_Input(LoadButton,function()
				task.spawn(ConfigButton.OnLoad);
			end);

			Noir:_Input(SaveButton,function()
		...
		function TabArgs:Init()
			...
			Noir:_Input(Button,function()
				if TextBox.Text:byte() then
					WindowArgs.Notify.new({
						Title = "Configs",
						Icon = Noir:_GetIcon(Config.Logo),
			...
			local Refresh = function()
				...
				for i,v in next , FullConfig do
					...
					Button.OnLoad = function()
						WindowArgs.Notify.new({
							Title = "Configs",
							Icon = Noir:CacheImage(Config.Logo),
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		if Internal then
			...
			TabContent.Name = Noir:_RandomString()
			...
			Internal.Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				...
				if Noir.PerformanceMode then
					if TabContent.Visible then
						Noir:_SetNilP(TabContent , Internal.Parent);
					else
						Noir:_SetNilP(TabContent , nil);
					end;
				else
					Noir:_SetNilP(TabContent , Internal.Parent);
				end;
			...
			if Noir:_IsMobile() then
				Noir:_AddDragBlacklist(Left);
				Noir:_AddDragBlacklist(Right);
			end;
			...
			Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				...
				if Noir.PerformanceMode then
					if TabContent.Visible then
						Noir:_SetNilP(TabContent , TabMainFrame);
					else
						Noir:_SetNilP(TabContent , nil);
					end;
				else
		...
		function TabArgs:DrawSection(config: Section)
			...
			Section.Name = Noir:_RandomString()
			...
			Section.BackgroundColor3 = Noir.Colors.BlockColor
			...
			if Noir:_IsMobile() then
				Noir:_AddDragBlacklist(Section);
			end;
			...
			local refresh = function(Upvalue)
				...
				if IsOpen then
					...
					if (Section:GetAttribute('Height') and not Noir:_IsMobile() and FullScale <= RefPos) then
						Noir:_Animation(Section,TweenInfo.new(0.4,Enum.EasingStyle.Quint),{
			...
			TabOpenSignal:Connect(function(bool)
				if bool then
					Noir:_Animation(Section,TweenInfo.new(0.21),{
						BackgroundTransparency = 0
					})

					Noir:_Animation(SectionText,TweenInfo.new(0.21),{
						TextTransparency = 0.500
					})

					Noir:_Animation(SectionClose,TweenInfo.new(0.21),{
						ImageTransparency = 0.500
					})
				else
					Noir:_Animation(Section,TweenInfo.new(0.21),{
			...
			Noir:_Input(Header,function()
				IsOpen = not IsOpen;

				if IsOpen then
					Noir:_Animation(SectionClose,TweenInfo.new(0.35),{
						Rotation = 0
					});
				else
					Noir:_Animation(SectionClose,TweenInfo.new(0.35),{
			...
			Header.MouseEnter:Connect(function()
				Noir:_Animation(SectionText,TweenInfo.new(0.2),{
			...
			return Noir:_LoadElement(Section , true , TabOpenSignal)
		end;
	...
	do
		...
		CloseWindow.Name = Noir:_RandomString()
		CloseWindow.Parent = CompKiller
		CloseWindow.AnchorPoint = Vector2.new(1, 0)
		CloseWindow.BackgroundColor3 = Noir.Colors.BGDBColor
		...
		local ToggleCloseUI = function(v)
			...
			if v then
				...
				Noir:_Animation(CloseWindow,TweenInfo.new(0.2),{
		...
		function WindowArgs:Watermark()
			local Signal = Noir.__SIGNAL(true);
			...
			Watermark.Name = Noir:_RandomString()
			Watermark.Parent = CompKiller
			Watermark.AnchorPoint = Vector2.new(1, 0)
			Watermark.BackgroundColor3 = Noir.Colors.BGDBColor

			Noir:Drag(Watermark , Watermark, 0.1);
			...
			Logo.Name = Noir:_RandomString()
			Logo.Parent = Watermark
			Logo.AnchorPoint = Vector2.new(1, 0.5)
			Logo.BackgroundColor3 = Noir.Colors.BGDBColor
			...
			WaternarkList.Name = Noir:_RandomString()
			...
			BackFrame.Name = Noir:_RandomString()
			...
			Noir:_Blur(BackFrame,Signal);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Noir:_Animation(Watermark,TweenInfo.new(0.4),{
			...
			function Args:AddText(Watermark : Watermark)
				Watermark = Noir.__CONFIG(Watermark, {
					Text = "Watermark",
					Icon = "info"
				});
				...
				Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
				Icon.ZIndex = 156
				Icon.Image = Noir:_GetIcon(Watermark.Icon);
				...
				function Arg:Visible(v)
					...
					if Noir.PerformanceMode then
						if v then
							Noir:_SetNilP(Icon , WaternarkList);
							Noir:_SetNilP(TextLabel , WaternarkList);
						else
							Noir:_SetNilP(Icon , nil);
							Noir:_SetNilP(TextLabel , nil);
						end;
					else
						Noir:_SetNilP(Icon , WaternarkList);
						Noir:_SetNilP(TextLabel , WaternarkList);
		...
		local Button = Noir:_Input(CloseWindow,function()
			WindowArgs:_ToggleUI()
		end)

		if not Noir:_IsMobile() then

			Noir:_Hover(Button,function()
	...
	function WindowArgs:Update(config: WindowUpdate)
		...
		if Noir.SecureMode and string.find(config.UserProfile, "rbxassetid://",1,true) then
			config.UserProfile = Noir:_GetIcon("user");
		end;
	...
	WindowArgs.LOOP_THREAD = task.spawn(function()
		...
		BlurElement.Name = Noir:_RandomString()
		...
		Noir:_Blur(BlurElement , WindowOpen);

		local MovementFrame = Instance.new("Frame")

		MovementFrame.Name = Noir:_RandomString()
		...
		Noir:Drag(MovementFrame,MainFrame,0.1)
		...
		while true do task.wait(0.01);
			...
			if WindowArgs.SelectedTab and WindowArgs.IsOpen then
				...
				if vili < 0 or vili > TabButtons.AbsoluteSize.Y then
					Noir:_Animation(SelectionFrame , TweenInfo.new(0.1) , {
						BackgroundTransparency = 1
					});
				else
					if math.abs(distance) <= 10 then
						...
						Noir:_Animation(SelectionFrame , TweenInfo.new(0.15) , {
							BackgroundTransparency = 0,
...
function Noir:GetConfig(Type: string)
	local ConfigFlags = {};
...
function Noir:ConfigManager(ConfigManager: ConfigManager) : ConfigFunctions
	ConfigManager = Noir.__CONFIG(ConfigManager , {
		Directory = "NoirUI",
		Config = "Software"
	});
	...
	if not isfolder(Noir:_Path(ConfigManager.Directory , ConfigManager.Config)) then
		makefolder(Noir:_Path(ConfigManager.Directory , ConfigManager.Config));
	end;

	local Args = {
		Directory = Noir:_Path(ConfigManager.Directory , ConfigManager.Config);
		EnableNotify = false,
	};
	...
	function Args:WriteConfig(Config: WriteConfig)
		Config = Noir.__CONFIG(Config , {
			Name = Noir:_RandomString(),
			Author = LocalPlayer.Name,
		});

		local Flags = Noir:GetConfig("MK");

		Flags["__INFORMATION"] = {
			Type = "Information",
			Author = Config.Author,
			Name = Config.Name,
			CreatedDate = Noir:GetDate()
		};

		if Args.EnableNotify then
			notify.new({
				Title = "Configs",
				Icon = Noir:_GetIcon('settings'),
				Content = "Create config \""..Config.Name.."\""
			})
		end

		writefile(Noir:_Path(Args.Directory , Config.Name) , HttpService:JSONEncode(Flags));
	end;

	function Args:LoadConfigFromString(str: string)
		local decoded = HttpService:JSONDecode(str);

		local Flags = Noir:GetConfig("KV");
	...
	function Args:GetCurrentConfig()
		return Noir:GetConfig("MK")
	end;

	function Args:ReadInfo(ConfigName: string)
		local _path = Noir:_Path(Args.Directory , ConfigName);
...
function Noir:Loader(IconId,Duration)
	local CompKiller = Instance.new("ScreenGui")

	CompKiller.Name = Noir:_RandomString()
	CompKiller.Parent = CoreGui
	...
	Loader.Name = Noir:_RandomString()
	...
	Icon.ZIndex = 100
	Icon.Image = IconId or Noir.Logo;
	Icon.ImageTransparency = 1

	Vignette.Name = Noir:_RandomString()
	Vignette.Parent = Loader
	Vignette.BackgroundTransparency = 1.000
	Vignette.BorderColor3 = Color3.fromRGB(27, 42, 53)
	Vignette.BorderSizePixel = 0
	Vignette.Size = UDim2.new(1, 0, 1, 0)
	Vignette.Image = Noir:CacheImage("rbxassetid://18720640102")
	Vignette.ImageColor3 = Noir.Colors.Highlight
	Vignette.ImageTransparency = 1
	Vignette.AnchorPoint = Vector2.new(0.5,0.5)
	Vignette.Position = UDim2.fromScale(0.5,0.5)

	Noir:_Animation(Loader,TweenInfo.new(0.55,Enum.EasingStyle.Quint),{
		BackgroundTransparency = 0.5
	});
	...
	task.delay(0.5,function()
		Noir:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint),{
			ImageTransparency = 0.01,
			Size = UDim2.new(0, 200, 0, 200)
		});

		task.delay(0.25,function()
			Noir:_Animation(Vignette,TweenInfo.new(5),{
				ImageTransparency = 0.2
			});

			task.wait(Duration or 4.5)

			Noir:_Animation(Vignette,TweenInfo.new(3,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				Size = UDim2.new(2, 0, 2, 0)
			});

			Noir:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				ImageTransparency = 1,
			});

			Noir:_Animation(Loader,TweenInfo.new(1.5,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				BackgroundTransparency = 1
			});

			task.delay(0.1,function()
				Noir:_Animation(Vignette,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
					ImageTransparency = 1
				});
...
function Noir.newNotify()
	if Noir.NOTIFY_CACHE then
		return Noir.NOTIFY_CACHE;
	end;
	...
	Notification.Name = Noir:_RandomString()
	Notification.Parent = CoreGui;
	Notification.ResetOnSpawn = false
	Notification.ZIndexBehavior = Enum.ZIndexBehavior.Global

	NotifyContainer.Name = Noir:_RandomString()
	...
	Noir.NOTIFY_CACHE = {
		new = function(Notify: Notify) : NotifyPayback
			...
			BlockFrame.Name = Noir:_RandomString()
			BlockFrame.Parent = NotifyContainer
			BlockFrame.AnchorPoint = Vector2.new(1, 0)
			BlockFrame.BackgroundColor3 = Noir.Colors.BGDBColor
			...
			NotifyFrame.Name = Noir:_RandomString()
			NotifyFrame.Parent = BlockFrame
			NotifyFrame.BackgroundColor3 = Noir.Colors.BGDBColor
			...
			if string.find(Notify.Icon,'cache-ck-',1,true) then	
				...
				CompLogo.Image = Noir:_GetIcon(Notify.Icon);
			end;
			...
			Header.TextColor3 = Noir.Colors.SwitchColor
			Header.TextSize = 14.000
			Header.TextXAlignment = Enum.TextXAlignment.Left

			Body.Name = Noir:_RandomString()
			...
			Body.TextColor3 = Noir.Colors.SwitchColor
			Body.TextSize = 12.000
			Body.TextTransparency = 0.500
			Body.TextXAlignment = Enum.TextXAlignment.Left
			Body.TextYAlignment = Enum.TextYAlignment.Top

			TimeLeftFrame.Name = Noir:_RandomString()
			...
			TimeLeft.Name = Noir:_RandomString()
			TimeLeft.Parent = TimeLeftFrame
			TimeLeft.BackgroundColor3 = Noir.Colors.Highlight
			...
			local UpdateText = function()
				...
				if BlockFrame:GetAttribute('Already') then
					Noir:_Animation(BlockFrame,TweenInfo.new(0.3),{
			...
			local Close = function()
				Noir:_Animation(NotifyFrame,TweenInfo.new(0.65,Enum.EasingStyle.Quint),{
			...
			local Show = function()
				Noir:_Animation(NotifyFrame,TweenInfo.new(0.5,Enum.EasingStyle.Quint),{
					Position = UDim2.new(0,0,0,0)
				});
			end;

			if typeof(Notify.Duration) == 'number' and Notify.Duration ~= math.huge then
				Noir:_Animation(TimeLeft,TweenInfo.new(Notify.Duration + 0.2,Enum.EasingStyle.Linear),{
			...
			return {
				...
				SetProgress = function(self , new , Time)
					...
					Noir:_Animation(TimeLeft,TweenInfo.new(Time or 0.85,(Time and Enum.EasingStyle.Linear) or Enum.EasingStyle.Quint),{
...
Noir.NilFolder.Name = "Nil-Instances";
